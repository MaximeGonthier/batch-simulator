...
\section{Todo}
	...
	\subsection{Uppsala}
		\begin{enumerate}
			\item plot et montrer cluster usage ainsi que stat du workload avec les perfs à côté
			\item tester avec data sur jobs de plus de 5 cores seulement
			\item la version omnisicnet de area filling devrais marcher sur un exemple simple avec des ratios que je maitrise = verifier les perfs sur les gros jobs car la moyenne peut tout lisser
			\item backfill big node avec fcfs score: faire a la place si dans taille + 1 je bats la moyenne des scores precedant de 50\% au moin je peux le compute dessus. + voir area filling aussi puis tester avec contraintes de taille seulement et faire varier les proportions dans le workload
