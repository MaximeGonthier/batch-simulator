...
\section{Todo}
	...
	\subsection{Uppsala}
		\begin{enumerate}
			...
			\item retester avec V10000
			\item tester avec plus de jours
			\item refaire cluster usage en allant lus loin dans les jours d'après, en coupant a une certine time frame et en metant les courbes de queue au dessus (en faisant juste la somme) et en pointillé meme couleur que node used.
			\item calculer pourcentage avec juste valeurfcfs/valeur a comparer
			\item plot speedup all workloads
			\item plot speedupen nuage de points avec les jours en abcsiscce
			\item dans l'article mettre exemple de 1 ou 2 jours + wuleter usage + pourcentgae all + nuage de points
