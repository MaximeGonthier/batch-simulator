...
\section{What's new since last meeting ?}
	...
	\begin{enumerate}
		...
		\item J'ai refais toutes les visu en divisant par 2
		\item plot sparse 10\%. On a aussi 2\%.
		\item regarder papier de l'auteur Celerity-SYCL-2019.pdf
	\end{enumerate}
	
\section{Questions for next meeting}
	\begin{enumerate}
		\item il faudra refaire le dépot ?
		\item nouvelle fig 11, si elle est ok je fais ca pour tous.
	\end{enumerate}
...
\section{Todo}
	...
	\subsection{Uppsala}
		\begin{enumerate}
			...
			\item corriger alignement table 1
			\item appeller dmda comme un state of the art result pour le produit de matrice l'appeller "chameleon gemm" et citer un papier chamelon et son noyau gemm
			\item ajouter une ref à Parla et MICCO ?
			\item expliquer visu
			\item insérer sparse 10\%
			\item se renseigner SOOP et si y a rien le dire dans l'article
			\item 29, a la place de automate dire allow to direct et vérifier
			\item corriger refinded
			\item revoir la partie ovverlap section PGAS et vérifier
			\item finir QR, tester QR
			\item une fois les visus remplacé et validé, modifier le texte correspondant et les captions et répondre au reviewer.
			\item commencer a compléter la selection related work: data locality in runtime et data locality for gpus
			\item paragrahphe sur mapping legion hpx chapel mais lonins de nous car monogpu dans notre cas. Regarder librairie PHAST et celerity.
			\item lire papiers sur des runtime
