...
\section{What's new since last meeting ?}
	...
	\begin{enumerate}
		...
		\item Adapter texte a suppresion de sched time, MST et HFP only ready.
		\item Texte sur Sparse et modif de conclusions
		
		% Uppsala
		\item Code pour faire workload sparse
		\item Test sur des workloads dense ou sparse
		\item pseudo code de l'algo mix. Debut de test mais complexité élevé car jannule pas des le EAT.
	\end{enumerate}
...
\section{Questions for next meeting}
	\begin{enumerate}
		...
		\item J'ai enleveé MST des explications aussi. devrais je le garder sans le montrer sur la courbe? par contre je l'ai gardé des \% daméliorations
		\item faire la mix strategy mais quand même avec le EAT pour couper la complexité ?
	\end{enumerate}
