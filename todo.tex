...
\section{Questions for next meeting}
	\begin{enumerate}
		\item Verif ce qu'on a fais sur: "by design the data is the right size" au moment de 
		"We consider that the memory of a node, denoted by M is
		only used by the jobs’ input files, since all other data are
		negligible compared with the input files. We assume that this
		memory is large enough to fit all input the files of jobs running
		on the node" dans le framework.
		\item Verif fusion des deux mentions de Stretch et flow et retirer celle de la section 6
		\item On peut garder stretch speed-up dans la légende des figures VS car c'est job par job ?
		\item Pas 100\% convaincu par improvement à la place de speed-ups. Peut-être que je dois réécrirer en parlant de \% d'améliorations ?
		\item pas convaincu par les points a la place des barres aussi poir le stretch
		\item for load time relative to FCFS I do algo_load_time/FCFS_load_time to have the lower the better, is it correct?
		\item j'ai groupé les stretch et load time. On veut aussi les nb of re-use groupé ? A part ? Ou pas du tout je les mentionne juste?
		\item sur les stretch times, d'une fig a l'autres le max de y ne sont pas les mêmes
		\item sur les load times, d'une fig a l'autres les axes y sont les mêmes
		\item pas convaincu des points surtout sur la figure du 28 janvier
		\item fusionner section 5 et settings de section 6 ?
		\item voir ce que Loris veut écrire pour le "The vast majority of jobs on these resources are single-node jobs"
	\end{enumerate}
...
\section{Todo}
	...
	\subsection{Uppsala}
		\begin{enumerate}
			\item parler des jobs divisé en job single node au moment de la section 5 je crois
			\item teester courbe combiné carré vide pour bf
			%~ maxjp.gr2@gmail.com Requingithubccgrid2 user-for-double-blind-submission
			\item une fois finale, changer les -> en - dans les noms des figures du tex et des figs elle mêmes du coup
			\item dans section 5, dire que on utilise des logs de vrai jobs qui corresponde a un vrai cluster qui a le mm nb de cores et noeuds que ce su'on simule la
			\item modifier texte section 6 en fonction des nouvelles figures
			\item corriger texte ou je parle d'improvement avec "with 25\% of those results with an improvement greater than 35\%"
			\item mettre le texte au présent dans les sections 5 et 6 surtout. Enlever donc le futur ( "They will be attributed" -> "They are attributed..") et le passé et éviter la voix passive (ex: Every year thousands of people are killed on our roads.)
			\item Ajouter justification de multiplicateur *500 (nb total de noeuds?) dans section 4.B
			\item Soumettre le 12 décembre
			\item Verifier comment on parle du bf et peut etre mettre: "Backfilling allows smaller jobs
			lying farther back in the queue to be scheduled before the other jobs waiting at the front
			of the queue, as long as the jobs at the front are not delayed by this decision"
			...
			\item Aussi il y a quelque chose que je n'ai jamais utilisé dans les explications c'est la courbe des cores qui chargent un fichiers sur les courbes d'utilisation du cluster, si jamais vous avez des idées/encdroit où je devrais l'utiliser pour parler de nos stratégies ?
