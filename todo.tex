...
\section{What's new since last meeting ?}
	...
	\begin{enumerate}
		\item Reduced complexity on nb of copy and some tests
		\item stretch with a minimum I do: bounded\_stretch = $t/max(t\_empty\_cluster, 5min)$
		\item Rewritting of area filling algorithm
		\item some heatmaps
	\end{enumerate}
...
\section{Questions for next meeting}
	...
	\begin{enumerate}
		\item Sort also for fcfs and fcfs no use bigger node the job list by file size ? It's not done in real.
		\item Area filling: what really is stat from previous workload ? Depending on the amount of job I look at, the area allowed in planned area change
	\end{enumerate}

\section{Todo}
	\subsection{General}
		\begin{enumerate}
			\item ignore dt for constraint = 1
			\item area filing: veriff avec loris ce que j'ai ecrit puis le coder
			\item are filling omnisicnet comme je fais en ce moment. Si ca marche pas essayer avec le graduellement
			\item try sort by size for fcfs and fcfs no use bigger nodes. Do with and without. + other algo already tested before
			
		
			\item générer stats omnisicnet et normal area filling
			\item Some heatmaps (max flow, total flow, max stretch, mean stretch, max bounded stretch, mean bounded stretch) for fcfs with a score
			\item poser congés
			\item coder area filling en suivant ce qu'il y a dans le .pdf
