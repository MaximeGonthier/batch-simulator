...
\section{What's new since last meeting ?}
	...
	\begin{enumerate}
		...
		\item With 3 phase I am on a very intensive part of the workload with similar submission times and lot of jobs because I ignore the start which is a part of older execution. But for now I just divide in 3 the workload by submission time. Maybe I can just cut the first and last 10\% ? That's what I did for the last results.
		\item stats on 3 phases workload : cores used, node used, delay and walltime distribution, size of queue.
		...
		\item Idea of strat to deal with different sizes of files requirement
	\end{enumerate}
	
\section{Questions for next meeting}

	\begin{enumerate}
		\item New strategy ? Score variant ? Any other ideas of heuristics ?
		...
		\item Change percentage of task with 256 or 1024 files ? for now it's 80/10/10 
	\end{enumerate}
