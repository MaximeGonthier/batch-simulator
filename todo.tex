...
\section{Questions for next meeting}
	\begin{enumerate}
		\item Faire de multiplier evicted file un tie breaker seulement ? Pareil avec nb of copy en second tiebreaker ? Comme ça on enlève un peu ce côté mélange de scores. On peut meme l'ajouter au locality only comme tie breaker avant le EAT ? Voir le nb de tiebreaks.
		\item Je go pour FGCS (reste 1 todo)?
...
\section{Todo}
	...
	\subsection{FGCS}
		\begin{enumerate}
			\item upload, faire zip pour tex files marked et unmarked et enlever commentaires
		\end{enumerate}
	\subsection{Uppsala}
		\begin{enumerate}
			...
			\item ecrire pseudocode if EAT == t et revoir pseudocodes des autres algos que je montre
			\item trouver des meilleurs noms
			...
			\item compter les tie break pour fcfs score (hors evicted file du coup)
