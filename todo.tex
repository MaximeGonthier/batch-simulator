...
\section{Questions for next meeting}
	\begin{enumerate}
		\item contraintes sur les tailles ou je peux executer que sur ma taille de noeurd ? Car la je peux executer sur toutes les tailles meme si je suis troo gros.
		\item since we are in the case were a job can be computed on any node, maybe I should have only had jobs with a file of size 128 ? Here I have jobs with sie from 128 to 1024 but they can be computed on any node.
		\item le speedup que je montre c'est le pourcentage de reduction du stretch obtenu avec la formule: c'est correct ?
		\item On veut stretch ou stretch with a min ? Quel min ? Entre les 2 les résultats sont très similaires.
		\item Pas sûr de ce qu'ols veulent pour cluster usage (cluster usage, green on top of the rest ce qui va squeeze le bleu) ?
	\end{enumerate}
