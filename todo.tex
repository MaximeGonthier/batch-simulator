...
\section{What's new since last meeting ?}
	...
	\begin{enumerate}
		...
		% Reu FGCS
		\item En bleu ajout phrases re-formulées et des captions des tables
		\item nouvelle figure 4 + nouvelle description dans le texte + réponse au reviewer
		\item articles et notes sur parsec entre autres
		\item nouvelle figure 9 qu'en pensez vous ? Tout divisé par 2
	\end{enumerate}
	
\section{Questions for next meeting}
	\begin{enumerate}
		%~ \item Ajouter bf a tous et retester ?
		\item Data in Brief pour ajouter des pages en plus ?
		\item Que pensez vous de la nouvelle figure 4 ? Devrais-je mettre l'explication dans la caption plutot que dans le texte plus haut comme j'ai fais ?
		\item nouvelle figure 9 ?
		\item on a pas d'autres applications sans dépendances avec des données partagées ?
		\item Sparse 2\% avez été testé qu'en pensez vous ?
	\end{enumerate}
	
\section{Todo}
	\subsection{General}
		\begin{enumerate}
			\item agrandir barre blanches
			...
			\item att mail de delphine en telecomm car elle me dois 1500e: normalement je suis payé en octobre
		\end{enumerate}
	\subsection{Uppsala}
		\begin{enumerate}
			\item faire 2 workloads, 1 chargé et 1 non et tester Fcfs et Fcfs with a score. Si fcfs with a score s'en sort mal en cluster vide, il faut des multiplicateurs qui s'adaptent. et tester avec EATx500
			\item Tester x500 au EAT
			
			\item lire X10.pdf
			\item tester matrice sparse 10\% ?
			\item essayer de coder QR
			\item pour les runtime juste regarder ce qu'ils font pour la localité et l'eviction, et est-ce que ils font du GPU
			\item regarder hdr sam pour biblio runtimes
			\item refaire toutes les visu de la même manière. Une fois fais refaire les explications dans le texte et les captions et répodre au reviewer.
			\item il y a des runtime qui n'ont pas poussé mais qui ont lidée de re utilistion mais pas etudié: exemple parsec. L'idée est de mettre a la suite tous les gemms dispo sur cholesky. voir la bilbio si ils ont quelquechose la dessus. Georges Bosilca lui demander. L'implémenter ?
			\item si je trouve un scheduler de legion qui gere la localité des données, le tester. Regarder les articles et les sommaires des thèses.
			\item chiffre plus gros axe des visu pour tous. faire la figure plus petite mais avec des carré plus gors et accentuer le gradient.
			\item expliquer ce qu'on vois sur les captions de toutes les visualisations. Par exemple, la ligne ne change pas de luminosité alors que de haut en bas oui. Donc l'ordre est par ligne. Si j'explique dans la caption, enlever un peu dans le texte pour ne pas faire doublon.
			\item reviewer 1 point 1: "use like recent article library": we have independnet task only, so cholesky and marix are our best bet to compare our schedulers
			...
			\item parler dans le related work des runtime mentionné avec des articles à l'appui, et pourquoi ils sont diférents de notre approche. Legion en particulier. Parsec et plasma l'utilisateur place a la main la localité mais ne la prévois pas. : https://legion.stanford.edu/publications/ & https://legion.stanford.edu/publications/#soi_thesis
			\item on va dépasser les 18 pages. Demander si on a le droit.
			\item reviewer 1: sur 1 runtime c'est bcp d travail, donc encore plus si on veux comparer 3 runtimes.
			\item regarder dernier journaux cité par reviewers 1 des dernières années (https://dblp.org/db/conf/ics/ics2021.html) voir ce qui parle de localité/runtime/GPU parel pour SC et ipdps en plus précis que ce qu'ils nous ont fourni
			
			\item prep cours re216
