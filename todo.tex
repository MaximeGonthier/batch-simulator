...
\section{What's new since last meeting ?}
	...
	\begin{enumerate}
		...
		% Reu FGCS
		\item Citation QR-heft-tmdp-pr dans réponse au reviewer 2. A retirer.
		\item Revoir parla et optimal locality scheduling et refaire dans les related works
		\item expliquer sparse dans settings
		\item parler de dmdar comme default chameleon scheduler
		\item
		\item clarification single GPU dans abstract et intro. Faut-il en faire encore plus dans l'abstract en modifiant par exemple la première phrase ?
		\item avancement projet Suède: le point de basculement de charge c'est la quantité de job dans la queue (pret mais qui run pas). Mettre une pénalité constante sur les gros fichiers. tester x1 x1 et x1 x1 mais qui depend de l'utilisation du cluster. Tester la pénalité sur les gros jobs. 
		\item Tester en prenant le même jour et le rendre sparse.
...
\section{Questions for next meeting}
	\begin{enumerate}
		\item Garder N en abscisse des courbes sparses ? Mettre working set size
...
\section{Todo}
	\subsection{General}
		\begin{enumerate}
			...
			\item envoer certificat enseirb une fois validé
		\end{enumerate}
	\subsection{FGCS}
		\begin{enumerate}
			...
			\item fusionner fig 5 et 6. Mettre fond blanc à tous. Garder les points. Enlever fig13. Les 6 dernières, les mettres ne 2 colonnes 3 lignes. Alleger les schedulers. Enlever le HFP with sched time fast start de 3D. Simplifier les lignes horizontales et verticales et bien les expliquer avant dans le texte au moment de 2D. Garder only belady et only flip. Enlever mst.
			...
			\item enlever annotations lignes pour 6 courbes ensemble
		\end{enumerate}
	\subsection{Uppsala}
		\begin{enumerate}
			\item faire 2 workloads, 1 chargé et 1 non et tester Fcfs et Fcfs with a score. Si fcfs with a score s'en sort mal en cluster vide, il faut des multiplicateurs qui s'adaptent. et tester avec EATx500 ?? Non je pense pas pour ce dernier point car ce qu'on veut c'est la diff avec la autres, pas une emphase la dessus.
