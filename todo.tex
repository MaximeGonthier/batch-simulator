...
\section{What's new since last meeting ?}
	...
	\begin{enumerate}
		...
		\item Code Fcfs with a score + easy bf
		\item Diapos
		\item Gantt charts
		\item Expe full workload and reduce workload and cluster
		\item Now for each algo the schedule is fully redone at each liberated core. When new job arrive, we just schedule the new jobs.
	\end{enumerate}
	
\section{Questions for next meeting}

	\begin{enumerate}
		\item Almost no difference on the workload
		\item Stefanos
...
\section{Todo}
	\subsection{General}
		\begin{enumerate}
			...
			\item Pas besoin de bf pour le moment pour les autres, juste corriger le shiftleft
