...
\section{What's new since last meeting ?}
	...
	\begin{enumerate}
		...
		\item code data peristence avec LRU pour l'eviction
...
\section{Questions for next meeting}
	\begin{enumerate}
		...
		\item Quand j'ai le choix entre utiliser un trou ou non et que dans les 2 cas ca passe, je dois le faire ? Pour le moment si j'ai le choix, je ne vais pas dans le trou. Dnas mon code corrigé si j'ai le choix je vais dans le trou
...
\section{Todo}
	\subsection{General}
		\begin{enumerate}
			\item mettre les notes de 2A sur Thor
			\item mail sandrine enlever cours pendant sc et le donner à Denis: att réponse
	...
	\subsection{Uppsala}
		\begin{enumerate}
			\item début de code pour limiter reschedule conservatie bf fcfs only
			\item test conservative bf commencé hier, le finir sur grid 5K
			\itme test easy bf lancé dans la nuit
			\item Compter le nb de fois que la persistence est exploité: pour Janvier 17: 14 fois avec FCFS et 20 fois avec SCORE. pour Janvier 21: 8 fois avec FCFS et 29 fois avec SCORE. Avec conservativ bf et 17 et fcfs: avec easy bf et 17 et fcfs: 
			\item finir et tester conservative bf version optimisé
			\item tester easy bf, moins couteux à implémenter
			\item tester conservative avec une et des limite(s) sur le nb de jobs ou de cores reschedule
			\item Se comparer à EFT
