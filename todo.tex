...
\section{What's new since last meeting ?}
	...
	\begin{enumerate}
		...
		\item more heatmaps fcfs with a score
		\item Code for area filling ratio
	\end{enumerate}
	
\section{Questions for next meeting}

	\begin{enumerate}
		\item Allocated Area[choosen size][x] gets Allocated Area[choosen size][x] + Area(Ji) with Area(Ji) = cores*walltime. But should I do cores*(delay+transfer time) ?
	\end{enumerate}
