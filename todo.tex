...
\section{What's new since last meeting ?}
	...
	\begin{enumerate}
		...
		\item best th seems to be 80, retest with it
	\end{enumerate}
	
\section{Questions for next meeting}
	\begin{enumerate}
		\item speed-up por speed up or speedup?
...
\section{Todo}
	...
	\subsection{Uppsala}
		\begin{enumerate}
			\item imprimer billets bassin des lumières
			\item courbe de meilleur cluster usage
			\item faire abstract avant le 25
			\item tester avec plus de jours
			\item refaire cluster usage en allant plus loin dans les jours d'après, en coupant a une certine time frame et en metant les courbes de queue au dessus (en faisant juste la somme) et en pointillé meme couleur que node used.
			\item plot speedup en nuage de points avec les jours en abcsiscce
			\item dans l'article mettre exemple de 1 ou 2 jours + cluster usage + pourcentgae all + nuage de points
			\item montrer cluster usage fcfs et eft score mix
