...
\section{What's new since last meeting ?}
	...
	\begin{enumerate}
		...
		\item Dépôt mis à jour
		
		% Uppsala
		\item Correction eviction
		\item nouvelle strat avec temps moyen perdu mais moins bonne que fcfs score adaptative multiplier
	\end{enumerate}
	
\section{Questions for next meeting}
	\begin{enumerate}
		\item J'ai mis des majuscules aux noms d'appli comme lors de la présentation du benchmark dès qu'on les cites dans les figures et captions de tableaux etc...
		\item Enelever le bleu de l'intro et de l'abstract
		\item Citation pour chameleon page 6 quand on parle de DMDAR ?
		\item on veux garder le bleu sur les figurs qui ont changé ? Ou sur les captions qui ont changé ?
		\item Reu Jeudi 16h ?
		\item Strat multiplicateur degressif avec nb de tache restante ayant cette donnée
	\end{enumerate}
