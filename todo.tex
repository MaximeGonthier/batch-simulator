...
\section{What's new since last meeting ?}
	...
	\begin{enumerate}
		...
		% Uppsala
		\item I did area filling for size constraint (it was slighltly better than FCFS in some case, similar to FCFS in other cases). tried easy backfilling with fcfs but not far. Then went back to only the locality constraint because I saw limitations of FCFS with a score.
		\item Plots
		\item New way to show statistics and gantt charts
		\item New way to build a workload
	\end{enumerate}
	
\section{Questions for next meeting}
	\begin{enumerate}
		\item Busy cluster task by task or group of task scheduled by group of task scheduled ? because I could do it after scheduling a task that would start at time t and then switch strategy mid schedule.
		\item Pour flow adaptation, il faut le flow moyen donc flow total/nb de tache qui ont été exécuté ?
	\end{enumerate}
...
\section{Todo}
	...
	\subsection{FGCS}
		\begin{enumerate}
			\item rechnager cholesky en avec depedandaces
			\item relire
		\end{enumerate}
	\subsection{Uppsala}
		\begin{enumerate}
			\item faire 1 première exec heft qui calcule le flow moyen. Si on est en dessous à t on fais de la localité sinon heft. Le faire en pseudocode.
