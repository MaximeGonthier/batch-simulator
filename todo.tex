...
\section{Questions for next meeting}
	\begin{enumerate}
		\item Should we add the deviation on the scatter ? Because I don't have this data I would have to re run all workloads. Maybe I can do it for 1 or 2 strategys.
		\item cluster usage plot: I do *20 to have number of cores but then we have weird situations wer it does not touch the black line but you still have jobs in quuee because they doesn't fit. If we explain it it's ok ?
		\item speed-up or speed up or speedup?
		\item should we have a section on data pesistence, showing that it has a little impact here?
		\item For redability i removed the day giving extreme value to score from all 4 heursitics; Is it really what we want ? Or only remove it for score ? Or only crop the image ?
		\item on the boxplots, don't we want the whisker to be the same size, from one scheduler to another ?
	\end{enumerate}
...
\section{Todo}
	...
	\subsection{Uppsala}
		\begin{enumerate}
			\item boxplot de fig13
			\item ecdf: Fonction de répartition empirique a plot sur all workloads de fig13 et les autres
			\item cluster usage, mettre axe x different pour les pointillé en partant de 0
			\item cluster usage, essayer en mettant les vrai valeurs du nb de coeurs
			\item cluster usage pareil pour nb de cores en load
			\item a loris je met CNRS-Lyon Et sam UB-Inria
			\item soumettre abstract
			\item faire abstract avant le 25
			\item finir 03-23
