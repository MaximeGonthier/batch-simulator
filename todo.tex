...
\section{Questions for next meeting}
	\begin{enumerate}
		...
		\item plan une réu avec elizabeth ?
	\end{enumerate}
...
\section{Todo}
	...
	\subsection{Uppsala}
		\begin{enumerate}
			...
			\item il y a des runtime qui n'ont pas poussé mais qui ont lidée de re utilistion mais pas etudié: exemple parsec. L'idée est de mettre a la suite tous les gemms dispo sur cholesky. voir la bilbio si ils ont quelquechose la dessus. Georges Bosilca lui demander
			...
			\item fig 4, mettre 1 couleur dans les cases par phase + barres au dessus pour les charegeents avec M=4. Ne montrer que les chargements de lignes. Avec U on a une phase en moins.
			...
			\item reviewer 1: "use like recent article library": we have independnet task only, so cholesky and marix are our best bet to compare our schedulers
			...
			\item parler dans le related work des runtime mentionné avec des articles à l'appui, et pourquoi ils sont diférents de notre approche. Legion en particulier. Parsec et plasma l'utilisateur place a la main la localité mais ne la prévois pas. : https://legion.stanford.edu/publications/
			\item on va dépasser les 18 pages. Demander si on a le droit.
			\item modif de gramaire, phrases trop longues et caption sur les tableaux + modification de phrase. On ne met pas ca dans une couleur différente ? Mettre en bleu la phrase reformulé et peut etre en voir d'autres
