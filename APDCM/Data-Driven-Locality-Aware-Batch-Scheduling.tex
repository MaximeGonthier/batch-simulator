...
\author{\IEEEauthorblockN{Maxime Gonthier}
\IEEEauthorblockA{\textit{University of Chicago, ENS de Lyon \& INRIA} \\
\textit{Department of computer science} \\
Chicago, USA \\
mgonthier@uchicago.edu}
\and
\IEEEauthorblockN{Elisabeth Larsson}
\IEEEauthorblockA{\textit{Uppsala University} \\
\textit{Department of Information Technology} \\
Uppsala, Sweden \\
elisabeth.larsson@it.uu.se}
\and
\IEEEauthorblockN{Loris Marchal}
\IEEEauthorblockA{\textit{CNRS} \\
\textit{ENS de Lyon} \\
Lyon, France \\
loris.marchal@ens-lyon.fr}
\and
\IEEEauthorblockN{Carl Nettelblad}
\IEEEauthorblockA{\textit{Uppsala University} \\
\textit{Department of Information Technology} \\
Uppsala, Sweden \\
...
\IEEEauthorblockA{\textit{University of Bordeaux} \\
\textit{LaBRI} \\
Talence, France \\
...
We present a third strategy,
described in Algorithm~\ref{algo.lem},
called Locality and Eviction Mixed (LEM).
LEM takes a similar approach to
...
% Figure et explication sur la sous utilisation du cluster
...
%~ With a workload that does not fully utilize the cluster, the job queue is empty most of the time. % Now explanation of the results
...
%~ : there are queues of several thousands requested cores for the whole duration of the evaluated week.
...
%~ \end{tabular}
%~ }
%~ \caption{Percentage reduction in data transfer time relative to FCFS on the workload of week 43.}\label{tab.43}
%~ \end{table}

%~ As we can see on Table~\ref{tab.43}, LEA and LEM greatly reduces the transfer time.
