...
\paragraph{Intuition} Find the file shared the most among available jobs.
Schedule all jobs using this file on the node the less used or on a node already using this file.
\begin{algorithm}[H]
%~ \caption{Representation of the order of load, computation and eviction\label{algo.model}}
\hspace*{\algorithmicindent} \textbf{Input: Set of available jobs $\jobset$} \\
...
%~ \For{$j \in \jobset$}
	...
	%~ \ForEach {$J_i \in \jobset$}
		%~ \State 
		%~ \If {$D_j \notin S_{\mathit{in\_mem}}$}
			%~ \State Add $D_j$ to $S_{\mathit{in\_mem}}$
		%~ \EndIf	
	%~ \EndFor
	\State Compute task $T_i$
%~ \EndFor
\end{algorithmic}
\end{algorithm}
