...
\paragraph{Intuition} Only schedule jobs that can be started immediatly on big nodes.
...
\begin{enumerate}
	...
	\begin{enumerate}
		\item If $J_i$ can start immediatly on a $x$ node, start it there
		\item Else if $J_i$ can start immediatly on a $x+1$ node, start it there
		\item Else if $J_i$ can start immediatly on a $x+2$ node, start it there
		\item Else if $J_i$ can start immediatly on a $x+3$ node, start it there
		\item ...
...
\section{Experiments}
	...
	\caption{Workload 2022-03-16 - 1/4 of the cluster - May 11}\end{figure}
	...
	\begin{minipage}[b]{0.5\linewidth}\centering\includegraphics[width=1\linewidth]{MBSS/plot/2022-03-26->2022-03-26_Total_flow_95_128_4_256_1_1024_skip_maximum_use.pdf}\caption{Total flow}\vspace{4ex}\end{minipage}\caption{Workload 2022-03-16 - 1/4 of the cluster - Ignoring Maximu use single file - May 11}\end{figure}
