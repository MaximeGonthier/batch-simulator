...
In order to do this we first look at all the jobs that ended between day 0 and 8. This should be enough to cover the ending times of most of the jobs submitted on day 1 because most jobs do not last for more than 7 days.
We sort these jobs by submission time. All the jobs submitted on day 0 and before are scheduled but not considered in our evaluation.
Job submitted before day 0 gives us the state of the cluster on day 0. Jobs from day 0 are ignored to limit the effect of bulk of jobs we could get with jobs submitted before day 0. Jobs from day 0 to 8 are all scheduled but only jobs from day 1 are evaluated.
...
We compute statistics previous workloads:
\begin{itemize}
	...
	\item $T_{max} \gets \frac{Area(\jobset)}{K}$
	\item For each size $x$, compute: $Area\_Jobs(x) = \sum_{i=0}^{n} Area(J_i)~such~that~\memory(\inputs(J_i)) == x$
...
\begin{itemize}
	\item If $Area(x) > T_{max}$: Jobs of size $x$ will only be scheduled on nodes of size $x$. $Area(x)$ is the percentage of time reserved to jobs of size $x$.
	%~ \item Else: $Planned\_Area(x) = Area(x)$
				%~ If $Area(x - 1) + Area(x) < T_{max}$: $Planned\_area(x) += Area(x - 1)$
				%~ Else: $Planned\_area(x) += Subset\_Area(x - 1)$ such as $Subset\_Area(x - 1) = T_{max} - Planned\_area(x)$
				%~ Nodes of size $x$ will be filled proportionnaly to jobs size in $Planned\_Area(x)$
\end{itemize}
...
\begin{enumerate}
	...
	\begin{enumerate}
		\item $Area\_Jobs(1) = 15$ core time
		\item $Area\_Jobs(2) = 30$ core time
		\item $Area\_Jobs(3) = 2$ core time
		\item $Area(\jobset) = 47$
		\item $T_{max} = \frac{Area(\jobset)}{k_1 + k_2 + k_3} = 4,7$
	\end{enumerate}
	\item $Area(1) = \frac{Area\_Jobs(1)}{k_1} = 3$
	\item $Area(2) = \frac{Area\_Jobs(2)}{k_2} = 10$
	\item $Area(3) = \frac{Area\_Jobs(3)}{k_3} = 1$
	\item $Area(3) < T_{max}$ so we do:
	\begin{enumerate}
		\item $Area(3) + Area(2) > T_{max}$, so all jobs of size $3$ will be scheduled on nodes of size $3$ and an area of size $T_{max} - Area(3) = 3,7$ can be jobs of size $2$ on nodes of size $3$. If this area is filled by jobs of size $2$ or $3$, we stop scheduling jobs of size $2$ on nodes of size $3$. So we can write: $Planned\_Area(3) = [ 0, 3.7, \infty ]$.
	\end{enumerate}
	\item $Area(2) - 3,7 > T_{max}$, so only jobs of size $2$ can be scheduled on nodes of size $2$.
	\item $Area(1) <  T_{max}$ but there are no space on nodes of size $2$ and $3$, so jobs of size $1$ will only be scheduled on nodes of size $1$.
\end{enumerate}
...
\begin{enumerate}
	...
	\begin{enumerate}
		\item If $J_i$ can start immediately on a $x$ node, start it there
		\item Else if $J_i$ can start immediately on a $x+1$ node \textbf{and $Planned\_Area(x+1)[x] > 0$}, start it there and $Planned\_Area(x+1)[x] \gets Planned\_Area(x+1)[x] > 0 - Area(J_i)$. \todo[inline]{Do we keep this immediatly with our new method ?}
		\item Else if $J_i$ can start immediately on a $x+2$ node \textbf{and $Planned\_Area(x+1)[x] > 0$}, start it there and $Planned\_Area(x+2)[x] \gets Planned\_Area(x+1)[x] > 0 - Area(J_i)$.
		\item ...
		\item Else schedule $J_i$ on a $x$ node
	\end{enumerate}
\end{enumerate}
