...
\ForEach {$J_i \in \jobset$}
	\ForEach {$\Node{k} \in \nodeset$}
		...
		\State $score_{\Node{k}} \gets A + 500 \times B + 1 \times C$
	\EndFor
...
\ForEach {$J_i \in \jobset$}
	\ForEach {$\Node{k} \in \nodeset$}
		...
		\State $C \gets (\memory(\inputs(\Node{k})~that~end~before~A) \times \frac{\core(J_i)}{\core(\Node{k})})/\bandwidth$
		\State $score_{\Node{k}} \gets A + B\_multiplier \times B + C\_multiplier \times C$
		\If {$\exists \mathit{Cores\_Hole}(\Node{k}) \subset \core(\Node{k})~such~as~|\core(J_i)| \leq |\mathit{Cores\_Hole}(\Node{k})| \wedge t + \walltime(J_i) \leq \mathit{Cores\_Hole}(\Node{k})~next~start~time$} \Comment{C'est pas tout à fais vrai car un job peut utiliser des cores d'un trou et des cores qui ne sont pas dans un trou en même temps.}
			\State $A \gets t$
			\State $B \gets$ the time to load $\inputs(J_i)$ on $\Node{k}$ at time $A$
			\State $C \gets (\memory(\inputs(\Node{k})~that~end~before~A) \times \frac{\core(J_i)}{\core(\Node{k})})/\bandwidth$
