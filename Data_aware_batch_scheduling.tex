...
For each node, C = compute the amount of data that will not be used because of current job. So it correspond to the percentage of core the current job will occupy multiplied by the size of files that end just before the computation of current job. These files will need to be re loaded for other jobs.
...
For FCFS with a score variant I do: Score = A + X $\times$ B + X $\times$ (C/BW) + Y $\times$ D\\
X and Y have to be found experimentally.
...
OR\\
\todo[inline]{
Cluster: Get area of nodes of each size noted $area\_node\_size(x) = nb\_cores \times ?$ for each x.\\
