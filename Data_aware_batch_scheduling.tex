...
\begin{itemize}
	...
	\item If $Area(x) < T_{max}$:
	\begin{itemize}
		\item For $i$ in range $(x - 1, 0)$
		\begin{itemize}
			\item If $Area(x) + Area(x-1) + ... + Area(i) < T_{max}$:
			\begin{itemize}
				\item $Planned\_Area(x)[i] = Area(i)$
			\end{itemize}
			\item Else
			\begin{itemize}
				\item $Planned\_Area(x)[i] = T_{max} - (Area(x) + Area(x-1) + ... + Area(i+1))$
				\item break
			\end{itemize}		
		\end{itemize}
	\end{itemize}
...
\begin{enumerate}
	...
	\item $Area(M_1) <  T_{max}$ but there are no space on nodes of size $M_2$ and $M_3$, so jobs of size $M_1$ will only be scheduled on nodes of size $M_1$.
\end{enumerate}

\todo[inline]{Hard to write general case}
...
\Else
	\For {$next\_size$ in range $(x+1, l)$}
		\If {$Planned\_area(next\_size)[x] - Area(J_i) > 0$ \& $J_i$ can start immediatly on a node of size $next\_size$}
