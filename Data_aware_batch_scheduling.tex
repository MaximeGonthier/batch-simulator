...
some nodes will be quiped with 16 cores and other 20.}
...
\todo[inline]{About memory map. We could represent that with two layers of memory ?
One that represent memory map and the other one the rest of the memory.}
...
\begin{enumerate}
\item Eviction of files if the memory is saturated; \todo[inline]{Maxime: Can we control the eviction policy of files ? In simulation I can, in real can I ?}
...
$$
\todo[inline]{Maxime: Should this be in the objectives above ? Or the max queue time ?}
...
$$
\todo[inline]{Maxime: Should min $\#Corehours$ be a goal ?}
