...
We consider that if $\memory(\inputs(J_i)) > \memory(\Node{i})$,
$J_i$ can't be computed on $\Node{i}$.

If at time $t$, $\memory(\inputs(J_i) \notin \inputs(\Node{i}) + \inputs(\Node{i})) > \memory(\Node{i})$
$J_i$ can't be computed on $\Node{i}$ at time $t$.

For each $i$, $\memory(\inputs(J_i)) \leq 1TB$.
...
Each job using input files, at least one file is an unique file and one file is a shared file.
...
\begin{itemize}
	...
	\item $J_i$ and $J_j$ are computed on the same node and all the jobs computed on this node between the execution of $J_i$ and $J_j$ do not require input files.
\end{itemize}
...
\todo[inline]{If a job uses the full memory of a node but only one core. I cannot schedule anything else on the node before the termination of said job ?}

\subsection{Maximum use single file}
\paragraph{Intuition}
Find the file shared the most among available jobs.
Schedule all jobs using this file on the node the less used or on a node already using this file.
\begin{algorithm}[H]
...
\While{$\jobset != \emptyset$}
	...
	\ForEach {$J_i \in \jobset$ such as $F_j \in \inputs(J_i)$}
		\State Schedule $J_i$ on $\Node{i}$ on the earliest available core(s)
