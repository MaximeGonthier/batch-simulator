...
Each job $j$ is equiped with a file of size $\frac{M_i}{N} \times \core(J_j)$, $M_i \in \mathbb{M}$.
...
\textbf{Objective: Minimize the \flow{} stretch}
		...
		$$
			\textbf{Obj.}: \quad \mathit{minimize}~\sum_{i=0}^{n}\frac{\flow{J_i}}{\duration(J_i) + \frac{\memory(\inputs(J_i))}{\bandwidth}}
		$$
...
Let's say we want to evaluate jobs submitted between day 1 and 3. 
In order to do this we first look at all the jobs that ended between day 1 and 10. This should be enough to cover the ending times of most of the jobs submitted between day 1 and 3 because most jobs do not last for more than 7 days.
We sort these jobs by submission time. All the jobs submitted before day 1 are scheduled but not considered in our evaluation.
They are used to emulate the state of the cluster on day 1. All the jobs submitted between day 1 and 3 are scheduled and evaluated.
All the jobs submitted between day 4 and 10 are not scheduled. \todo[inline]{Max: Or should they ?}
