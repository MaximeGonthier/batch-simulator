...
D = compute the number of copy (and thus the time it took to load them all) of the file you want to load on other nodes at the predicted time you want to compute the job.
...
%~ For each $x$, if $percentage\_node\_size(x) < percentage\_job\_size(x)$, then we scale the walltime for this size of node to $\walltime(J_i) + \walltime(J_i) \times (percentage\_job\_size(x) - percentage\_node\_size(x))$.}
%~ OR\\
%~ \todo[inline]{Cluster: Get area of nodes of each size noted $area\_node\_size(x) = nb\_cores \times ?$ for each x.\\
%~ Previous workloads: Get the area of jobs demanding a certain size noted $area\_job\_size(x) = nb\_cores \times Duration(J_i)$.\\
%~ }

We compute statistics previous workloads \todo[inline]{(is it coherent to get statistics on previous workloads or it's better to just get statistics on already processed jobs of the current workload ?)}.\\
For each job we compute it's usage area with $Area(J_i) = \core(J_i) \times \walltime(J_i)$.\\
For each size $x$, compute: $Area(x) = \sum_{i=0}^{n} Area(J_i)~such~that~\memory(\inputs(J_i)) == x$.\\
For each size $x$, compute: $Area\_ratio(x) = \frac{Area(x)}{Area(\mathbb{J})}$.
$Area\_ratio(x)$ is a number between 0 and 1 and gives us information about the use of a certain node's size.
We can try and use it as follow:

\begin{enumerate}
	\item Sort available job list by size of required node (biggest to smallest) and then by submission time
	\item Need to schedule job $J_i$, requiring a node of size $x$
	\item \textbf{$y \gets max(0, Mean\_queue\_time\_after\_i\_jobs - Queue\_time(J_i)) + \walltime(J_i) \times Area\_ratio(x)$}
	\begin{enumerate}
		\item If $J_i$ can start immediately on a $x$ node, start it there
...
\section{Experiments}
	...
	\begin{minipage}[b]{0.5\linewidth}\centering\includegraphics[width=1\linewidth]{MBSS/plot/2022-03-26->2022-03-26_Fcfs_Nb_scheduled_jobs_95_128_4_256_1_1024.pdf}\caption{Jobs in queue}\vspace{4ex}\end{minipage}\caption{Workload 2022-03-26 - Cluster usage - 1/4 cluster - May 30}\end{figure}
