...
\paragraph{Intuition}
%~ Select job submitted the earliest. Takes all available jobs using the file from this job. Do this with all available jobs.
%~ Once you have these packages of jobs, divide them. 
%~ You divide the sum of the number of cores asked by available jobs by the total number of cores. The result is the maximum number of cores that can be asked by one package.
%~ A full package is scheduled on a single node.
%~ Schedule packages in the order in which you created them (so sorted by submission time of the first job selected) on the node with the highest score (score of a node is computed just like fcfs with a score).
...
the time frame is the Mean queue time minus the time that already waited the current job plus the walltime of the job.
A job started immediately must not delay other jobs.
Let's note $x$ the size of a node.
\todo[inline]{Max: In order to scale the impact of walltime we can compute statistics on the cluster and the previous workloads.\\
Cluster: Get \% of nodes of each size noted $percentage\_node\_size(x)$ for each x.\\
Previous workloads: Get the \% of jobs demanding a certain size noted $percentage\_job\_size(x)$.\\
For each $x$, if $percentage\_node\_size(x) < percentage\_job\_size(x)$, then we scale the walltime for this size of node to $\walltime(J_i) \times (percentage\_job\_size(x) - percentage\_node\_size(x))$.}
\begin{enumerate}
	\item Sort available job list by size of required node (biggest to smallest) and then by submission time
	\item Need to schedule job $J_i$, requiring a node of size $x$
	\item \textbf{$y \gets Mean\_queue\_time\_after\_i\_jobs - Queue\_time(J_i) + \walltime(J_i)$}
	\begin{enumerate}
		\item If $J_i$ can start immediately on a $x$ node, start it there
		...
		\item Else if $J_i$ can start immediately on a $x+2$ node \textbf{and can't be started on an $x+1$ node before time $t + y$}, start it there
		\item Else if $J_i$ can start immediately on a $x+3$ node \textbf{and can't be started on an $x+2$ node before time $t + y$}, start it there
		\item ...
		\item Else schedule $J_i$ on a $x$ node
	\end{enumerate}
\end{enumerate}
...
\section{Experiments}
	...
	%~ \begin{figure}[H]\begin{minipage}[b]{0.5\linewidth}\centering\includegraphics[width=1\linewidth]{MBSS/plot/2022-03-16_Total_queue_time450_128_32_256_4_1024.pdf}\caption{Total queue time}\vspace{4ex}\end{minipage}\begin{minipage}[b]{0.5\linewidth}\centering\includegraphics[width=1\linewidth]{MBSS/plot/2022-03-16_Total_flow450_128_32_256_4_1024.pdf}\caption{Total flow}\vspace{4ex}\end{minipage}\caption{Workload 2022-03-16 - Full cluster - May 9}\end{figure}
	%~ \begin{figure}[H]\begin{minipage}[b]{0.5\linewidth}\centering\includegraphics[width=1\linewidth]{MBSS/plot/2022-03-26->2022-03-26_Fcfs_Used_nodes_450_128_32_256_4_1024.pdf}\caption{Used nodes}\vspace{4ex}\end{minipage}\begin{minipage}[b]{0.5\linewidth}\centering\includegraphics[width=1\linewidth]{MBSS/plot/2022-03-26->2022-03-26_Fcfs_Nb_scheduled_jobs_450_128_32_256_4_1024.pdf}\caption{Jobs in queue}\vspace{4ex}\end{minipage}\caption{Cluster usage - Full cluster - May 11}\end{figure}
	
	%~ \subsection{Full workload and 1/4 of the cluster and files on all jobs and no constraint on file sizes}
	%~ \begin{figure}[H]\begin{minipage}[b]{0.5\linewidth}\centering\includegraphics[width=1\linewidth]{MBSS/plot/2022-03-26->2022-03-26_Fcfs_Used_nodes_95_128_4_256_1_1024.pdf}\caption{Used nodes}\vspace{4ex}\end{minipage}\begin{minipage}[b]{0.5\linewidth}\centering\includegraphics[width=1\linewidth]{MBSS/plot/2022-03-26->2022-03-26_Fcfs_Nb_scheduled_jobs_95_128_4_256_1_1024.pdf}\caption{Jobs in queue}\vspace{4ex}\end{minipage}\caption{Workload 2022-03-26 - Cluster usage - 1/4 of the cluster - May 11}\end{figure}
	%~ \begin{figure}[H]\begin{minipage}[b]{0.5\linewidth}\centering\includegraphics[width=1\linewidth]{MBSS/plot/2022-03-26->2022-03-26_Total_queue_time_95_128_4_256_1_1024.pdf}\caption{Total queue time}\vspace{4ex}\end{minipage}%%
	%~ \begin{minipage}[b]{0.5\linewidth}\centering\includegraphics[width=1\linewidth]{MBSS/plot/2022-03-26->2022-03-26_Total_flow_95_128_4_256_1_1024.pdf}\caption{Total flow}\vspace{4ex}\end{minipage}
	%~ \begin{minipage}[b]{0.5\linewidth}\centering\includegraphics[width=1\linewidth]{MBSS/plot/2022-03-26->2022-03-26_Total_waiting_for_a_load_time_and_transfer_time_95_128_4_256_1_1024.pdf}\caption{Transfer + waiting for a load time}\vspace{4ex}\end{minipage}%%
	%~ \caption{Workload 2022-03-26 - 1/4 of the cluster - May 11}\end{figure}
	%~ \begin{figure}[H]\begin{minipage}[b]{0.5\linewidth}\centering\includegraphics[width=1\linewidth]{MBSS/plot/2022-03-26->2022-03-26_Total_queue_time_95_128_4_256_1_1024_skip_maximum_use.pdf}\caption{Total queue time}\vspace{4ex}\end{minipage}%%
	%~ \begin{minipage}[b]{0.5\linewidth}\centering\includegraphics[width=1\linewidth]{MBSS/plot/2022-03-26->2022-03-26_Total_flow_95_128_4_256_1_1024_skip_maximum_use.pdf}\caption{Total flow}\vspace{4ex}\end{minipage}\caption{Workload 2022-03-26 - 1/4 of the cluster - Ignoring Maximum use single file - May 11}\end{figure}
	...
	\begin{figure}[H]\begin{minipage}[b]{0.5\linewidth}\centering\includegraphics[width=1\linewidth]{MBSS/plot/FCFS_Score_2022-03-26->2022-03-26_Maximum_queue_time_95_128_4_256_1_1024.pdf}\caption{Maximum queue time}\vspace{4ex}\end{minipage}%%
	\begin{minipage}[b]{0.5\linewidth}\centering\includegraphics[width=1\linewidth]{MBSS/plot/FCFS_Score_2022-03-26->2022-03-26_Total_queue_time_95_128_4_256_1_1024.pdf}\caption{Total queue time}\vspace{4ex}\end{minipage}
	\begin{minipage}[b]{0.5\linewidth}\centering\includegraphics[width=1\linewidth]{MBSS/plot/FCFS_Score_2022-03-26->2022-03-26_Total_flow_95_128_4_256_1_1024.pdf}\caption{Total flow}\vspace{4ex}\end{minipage}%%
	\begin{minipage}[b]{0.5\linewidth}\centering\includegraphics[width=1\linewidth]{MBSS/plot/FCFS_Score_2022-03-26->2022-03-26_Total_waiting_for_a_load_time_and_transfer_time_95_128_4_256_1_1024.pdf}\caption{Waiting for a load and transfer time}\vspace{4ex}\end{minipage}\caption{Workload 2022-03-26 - Performance - 1/4 cluster - May 16}\end{figure}
