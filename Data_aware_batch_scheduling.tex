...
I suffer on underused parts of the workloads -> new mixed strategy for FCFS with a score:\\
\begin{algorithm}[htbp]
\caption{Mix SCORE/HEFT}
\hspace*{\algorithmicindent} \textbf{Input: Set of queued jobs $\jobset$. Set of nodes $\nodeset$.} \\
\begin{algorithmic}[0]
\State $occupation\_threshold \gets 99$ \Comment{How to define it ?}
...
\caption{Locality constraint only - Workload of March 2-3 (8000/47000 jobs) with resp. 85, 10 and 5\% of files of size 6.4, 12.8 and 51.2 MB per core - October 04}\end{figure}
...
New algorithm with flow adaptation related to the mean flow obtained with HEFT.\\
\begin{algorithm}[htbp]
\caption{FLOW ADAPTATION}
\hspace*{\algorithmicindent} \textbf{Input: Set of queued jobs $\jobset$. Set of nodes $\nodeset$.} \\
\begin{algorithmic}[0]
	\State Before the computation do: $Flow \gets$ HEFT($\jobset$, $\nodeset$) \Comment{insérer aussi l'idée de cluster occupé/vide ?}
	\While{$t$}
		\If{$Current\_Flow(t) \leq Flow(t)$}
			\State $Current\_Flow \gets$ LOCALITY($\jobset$, $\nodeset$)
		\Else
			\State $Current\_Flow \gets$ HEFT($\jobset$, $\nodeset$)
		\EndIf
	\EndWhile
\end{algorithmic}
\end{algorithm}
