...
We compute for each size: $Area(x) \gets \frac{Area\_Jobs(x)}{k_x}$ which correspond
...
For each size $x$ we make the following decision, starting with the biggest node size.

For each $x$ in decreasing order:
\begin{itemize}
	\item $Planned\_Area[x][x] \gets Area\_Jobs[x]$ \todo[inline]{I would rather write $Planned\_Area[x][x] \gets \infty$}
	...
	%~ \item If $Area(x) - Planned\_Area(x+1)[x] < T_{max}$:
	\item $y \gets x - 1$
	...
	\begin{itemize}
		...
		%~ \item For $i$ in range $(x - 1, 0)$
		%~ \begin{itemize}
			%~ \item If $Area(x) - Planned\_Area(x+1)[x] + Area(x-1) + ... + Area(i) \leq T_{max}$:
			%~ \begin{itemize}
				%~ \item $Planned\_Area(x)[i] = Area(i)$
			%~ \end{itemize}
			%~ \item Else
			%~ \begin{itemize}
				%~ \item $Planned\_Area(x)[i] = max(0, T_{max} - (Area(x) - Planned\_Area(x+1)[x] + Area(x-1) + ... + Area(i+1)))$
				%~ \item break
			%~ \end{itemize}		
		%~ \end{itemize}
	\end{itemize}
...
%~ %%% Initialisation
%~ - Remaining_Area[x] <- Tmax * nb_of_nodes[x]

%~ %%% On met ce qui reste des jobs de type x sur les machines x
%~ - Remaining_Area[x] <- Remaining_Area[x] - Area_Jobs[x]
%~ - Planned_Area[x][x] <- Area_Jobs[x]
%~ - Area_Jobs[x] <- 0

%~ %%% Tant qu'il reste de la place, on met alloue des jobs de type inférieur
%~ - y <- x-1
%~ - While Remaining_Area[x]>0 and y>=0:
  %~ - area\_to\_use <- min (Remaining_Area[x], Area_Jobs[y])
  %~ - Remaining_Area[x] <- Remaining_Area[x] - area\_to\_use
  %~ - Planned_Area[x][y] <- area\_to\_use
  %~ - Area_Jobs[y] <- - Area_Jobs[y] - area\_to\_use
 %~ (EndWhile)

%~ (EndFor)

Here is a small example. Let's say that we have $3$ sizes of nodes: $M_1, M_2$ and $M_3$, each having $k_1 = 5, k_2 = 3$ and $k_3 = 2$ nodes:
\begin{enumerate}
	...
	\end{enumerate}
	\item $Area(M_1) = \frac{Area\_Jobs(M_1)}{k_1} = 3$
	\item $Area(M_2) = \frac{Area\_Jobs(M_2)}{k_2} = 10$
	\item $Area(M_3) = \frac{Area\_Jobs(M_3)}{k_3} = 1$
	\item $Area(M_3) \leq T_{max}$ so we do:
	\begin{enumerate}
		\item $Area(M_3) + Area(M_2) > T_{max}$, so all jobs of size $M_3$ will be scheduled on nodes of size $M_3$ and an area of size $T_{max} - Area(M_3) = 3,7$ can be jobs of size $M_2$ on nodes of size $M_3$. If this area is filled by jobs of size $M_2$ or $M_3$, we stop scheduling jobs of size $M_2$ on nodes of size $M_3$. So we can write: $Planned\_Area(M_3) = [ 0, 3.7, \infty ]$.
	\end{enumerate}
	\item $Area(M_2) - 3,7 > T_{max}$, so only jobs of size $M_2$ can be scheduled on nodes of size $M_2$.
	\item $Area(M_1) \leq T_{max}$ but it's the smallest node size so it can only fit jobs from it's own size.
\end{enumerate}
...
%~ \If {$J_i$ can start immediately on a node of size $x$}
	%~ \State Start $J$ on a node of size $x$
	%~ \State return
%~ \Else
	%~ \For {$next\_size$ in range $(x+1, l)$}
		%~ \If {$Planned\_area(next\_size)[x] - Area(J_i) \geq 0$ \& $J_i$ can start immediatly on a node of size $next\_size$}
			%~ \State Start $J_i$ on a node of size $next\_size$
			%~ \State $Planned\_area(next\_size)[x] \gets Planned\_area(next\_size)[x] - Area(J_i)$
			%~ \State return
		%~ \EndIf
	%~ \EndFor
	%~ \State Schedule $J_i$ on a node of size $x$
	%~ \State return
%~ \EndIf		
%~ \end{algorithmic}
%~ \end{algorithm}
...
\State Need to schedule job $J_i$, requiring a node of size $x$
\For {$j$ in range $(x, l)$}
	\If {$Planned\_area(next\_size)[x] - Area(J_i) \geq 0$ or $j == x$}
		\State $EAT \gets$ earliest available time to start $J_i$ on a node of size $j$
		\If {$EAT < min$}
			\State $min \gets EAT$
			\State $Choosen\_size = j$
		\EndIf
	\EndIf
\EndFor
\State Start $J_i$ on a node of size $Choosen\_size$
...
\begin{figure}[H]\centering
\begin{subfigure}[b]{0.4\linewidth}\centering\includegraphics[width=1\linewidth]{MBSS/plot/Heatmap_Stretch_FCFS_Score_Time_to_load_file_Time_to_evict_file_2022-01-24->2022-01-24_450_128_32_256_4_1024.pdf}\caption{Stretch}\end{subfigure}
\begin{subfigure}[b]{0.4\linewidth}\centering\includegraphics[width=1\linewidth]{MBSS/plot/Heatmap_Stretch_with_a_minimum_FCFS_Score_Time_to_load_file_Time_to_evict_file_2022-01-24->2022-01-24_450_128_32_256_4_1024.pdf}\caption{Stretch with a minimum}\end{subfigure}
\begin{subfigure}[b]{0.4\linewidth}\centering\includegraphics[width=1\linewidth]{MBSS/plot/Heatmap_Total_Flow_FCFS_Score_Time_to_load_file_Time_to_evict_file_2022-01-24->2022-01-24_450_128_32_256_4_1024.pdf}\caption{Flow}\end{subfigure}\caption{June 28}\end{figure}
