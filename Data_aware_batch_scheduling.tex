...
\begin{itemize}
	...
	\end{itemize}
	\item For later experiments I will use fcfs with easybf and fcfs no use bigger nodes with easybf to compare myslef.
\end{itemize}
...
\paragraph{Idea for fcfs with a score backfill big nodes} 
The intuition is to use fcfs with a score. But If on a node of my size ($x$), the earliest available time is larger than t (or the mean queue time of previous jobs in mode 1), then I look at the score of nodes of size $x+1$. When I compute the score on bigger nodes I add a malus which is a multiplier times the area of the jobs (core used $\times$ walltime) compared to the total size. If I find a node where I can start at time t or (or the mean queue time of previous jobs in mode 1), I take the one with the best score. Else I go a size above.

We could also do that with the intuition of area filling: look from previous workload the allocated area available on bigger sizes and then compute a malus based on the amount of area I will occupy in this space. The issue is that with a clasic cluster, no area are allowed for smaller jobs.
