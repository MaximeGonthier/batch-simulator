...
\State Need to schedule job $J_i$, requiring a node of size $x$, at time $t$
\For {$next\_size$ in range $(x, l)$} \Comment{By starting at $x$, I assure that in the worst case I'm scheduled on a node of my size}
	\If {$Ratio\_Area[next\_size][x] * t - Allocated\_Area[next\_size][x] - Area(J_i) \geq 0~or~next\_size == x$}
