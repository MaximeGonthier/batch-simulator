...
/* Maxime */
Schedule::JobAlloc Schedule::add_job_first_fit_data_aware(const Job *job, ResourceSelector *selector, bool assert_insertion_successful)
{
    PPK_ASSERT_ERROR(!contains_job(job),
        "Invalid Schedule::add_job_first_fit call: Cannot add "
        "job '%s' because it is already in the schedule. %s",
    ...
    return add_job_first_fit_after_time_slice_data_aware(job, _profile.begin(), selector, assert_insertion_successful);
}

/* Maxime */
Schedule::JobAlloc Schedule::add_job_first_fit_after_time_slice_data_aware(const Job *job,
    std::list<TimeSlice>::iterator first_time_slice, ResourceSelector *selector, bool assert_insertion_successful)
{
	LOG_F(INFO, "Beggining of add_job_first_fit_after_time_slice_data_aware.");
    if (_debug)
    {
                     ...
                     ++pit2)
                {
                    ...
                    else if (totalTime >= job->walltime) // The job fits in the slices [pit, pit2[ (temporarily speaking)
                    {
                        ...
                        if (selector->fit(job, availableMachines, alloc->used_machines))
                        {
							...
							for (unsigned int i = 0; i < availableMachines.size(); i++)
							{
								LOG_F(INFO, "%d", availableMachines[i]);
							}
							
							/* Here I can choose a node! */
							//~ alloc->used_machines = availableMachines.first_element();
							//~ LOG_F(INFO, "Giving from available nodes %d", availableMachines.first_element());
