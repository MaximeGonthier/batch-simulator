...
public:
    ...
    //(new)
    void remove_from_available_machines(IntervalSet procs, TimeSliceIterator first_time_slice);
    void add_to_available_machines(IntervalSet procs, TimeSliceIterator first_time_slice);
    void add_failed_machine(int i);
    void remove_failed_machine(int i);
...
private:
    ...
    std::set <int> _failed_machines; // (new) add a new data_stucture set s to store failed machines
};
