...
ISchedulingAlgorithm::ISchedulingAlgorithm(Workload *workload, SchedulingDecision *decision, Queue *queue,
...
//,  map<string,double>&profile_dict
//, _profile_dict(profile_dict)
...
{
    (void)date;
    _jobs_released_recently.insert(_jobs_released_recently.end(), job_ids.begin(), job_ids.end());
}
...
{
    (void)date;
    _jobs_killed_recently.insert(_jobs_killed_recently.end(), job_ids.begin(), job_ids.end());
}
...
void ISchedulingAlgorithm::on_no_more_external_event_to_occur(double date) // new
{
    (void)date;
