...
/////////////////////////
// to be added in queue.cpp
FCFSPrioOrder::~FCFSPrioOrder()
{

}

bool FCFSPrioOrder::compare(const SortableJob * j1, const SortableJob * j2, const SortableJobOrder::CompareInformation *info) const
{
    //cerr<<"j1->job: "<<j1->job->id<<" j1->job->priority: "<<j1->job->priority<<" j1->release_date: "<<(double)j1->release_date<<endl;
    //cerr<<"j2->job: "<<j2->job->id<<" j2->job->priority: "<<j2->job->priority<<" j2->release_date: "<<(double)j2->release_date<<endl;
    (void) info;
    if (j1->job->priority == j2->job->priority) {
      if (j1->release_date == j2->release_date)
        return j1->job->id < j2->job->id;
      else
        return j1->release_date < j2->release_date; // job with smallest release date should go first
    } 
    else {
      return j1->job->priority > j2->job->priority; // job with largest priority should go first
    }
}

void FCFSPrioOrder::updateJob(SortableJob * job, const SortableJobOrder::UpdateInformation * info) const
{
    (void) job;
    (void) info;
}
...
{
    //cerr<<"remove job function succeed, the _jobs contains: ";
