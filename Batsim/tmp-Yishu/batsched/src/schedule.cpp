...
void Schedule::remove_from_available_machines(IntervalSet procs, TimeSliceIterator first_time_slice)
{
    //cerr<<"Schedule.cpp: remove_from_available_machines function is running"  << endl;
    for (auto it = procs.elements_begin(); it != procs.elements_end(); ++it) {  
            //cerr<<"processor "<< *it << " become unavailable, should be removed from the available machines"<<endl;
            for (auto pit = first_time_slice; pit != _profile.end(); ++pit) {
                // cerr<< "Remove processor " << *it<<" in the timeslice:  begin: " <<float(pit->begin) << " end: "<<float(pit->end) << " length: "<<float(pit->length) <<endl;          
                // cerr<<"The indexes of available machines previously: ";
                // int temp1=0;
                // for (auto it = pit->available_machines.elements_begin(); it != pit->available_machines.elements_end(); ++it) {  
                //     temp1 +=1;
                //     cerr << *it <<" ";
                //     if (temp1>9)
                //         break;
                // }
                //cerr<< " The number of available machines previously: "<< pit->nb_available_machines <<endl;
                if (pit->available_machines.contains(*it))
                {
                    //cerr<<"We did remove processor "<< *it<< endl;
                    pit->available_machines.remove(*it);
                    pit->nb_available_machines -= 1;
                }
                // cerr<<"The indexes of available machines now: ";
                // int temp2=0;
                // for (auto it = pit->available_machines.elements_begin(); it != pit->available_machines.elements_end(); ++it) {  
                //     temp2 +=1;
                //     cerr << *it <<" ";
                ...
                //         break;
                // }
                //cerr<< " The number of available machines now: "<< pit->nb_available_machines <<endl;
            }
    }
    //cerr<<endl;
}

void Schedule::add_to_available_machines(IntervalSet procs, TimeSliceIterator first_time_slice)
{
    //cerr<<"Schedule.cpp: add_to_available_machines function is running"  << endl;
    for (auto it = procs.elements_begin(); it != procs.elements_end(); ++it) {  
        //cerr<<"processor "<< *it << " become available, should be added to the available machines"<<endl;
        for (auto pit = first_time_slice; pit != _profile.end(); ++pit) {
            //cerr<<"Add processor " << *it<<" in the timeslice:  begin: " <<float(pit->begin) << " end: "<<float(pit->end) << " length: "<<float(pit->length) <<endl;       
            // cerr<<"The indexes of available machines previously: ";
            // int temp1=0;
            // for (auto it = pit->available_machines.elements_begin(); it != pit->available_machines.elements_end(); ++it) {  
            //         temp1 +=1;
            ...
            if (!pit->available_machines.contains(*it))
            {
                //cerr<<"We did add processor "<< *it<< endl;
                pit->available_machines.insert(*it);
                pit->nb_available_machines += 1;
            }
            // cerr<<"The indexes of available machines now: ";
            // int temp2=0;
...
void Schedule::add_failed_machine(int i)
{
    _failed_machines.insert(i);
//     cerr<<"Add processor "<<i<<" to the failed_machines" <<endl;
//     cerr<<"Now the failed_machines contains: ";
//     for (auto it = _failed_machines.begin(); it != _failed_machines.end(); ++it) {  
//             cerr << *it <<" ";
//     }
//     cerr<<endl;
}

void Schedule::remove_failed_machine(int i)
{
    _failed_machines.erase(i);
    // cerr<<"Remove processor "<<i<<" from the failed_machines"<<endl;
    // cerr<<"Now the failed_machines contains: ";
    // for (auto it = _failed_machines.begin(); it != _failed_machines.end(); ++it) {  
    //         cerr << *it <<" ";
    // }
    ...
    std::list<TimeSlice>::iterator first_time_slice, ResourceSelector *selector, bool assert_insertion_successful)
{
        ...
        if ((int)pit->nb_available_machines >= job->nb_requested_resources)
        {
            ...
            if (totalTime >= job->walltime)
            {
                ...
                if (selector->fit(job, pit->available_machines, alloc->used_machines))
                {
                    ...
                    //cerr<<"alloc->started_in_first_slice "<<alloc->started_in_first_slice<<endl;
                    alloc->job = job;
            ...
            else
            {
                     ...
                     ++pit2)
                {
                    ...
                    if (availableMachinesCount
                        < job->nb_requested_resources) // We don't have enough machines to run the job //print error message
                        break;
                    else if (totalTime
    ...
    //when the jobs in the queue rescheduled, reject the job whose number of requesed processors is larger than the number of current available processors 
    // cerr<<"_nb_machines "<<_nb_machines<<endl;
    // cerr<<"split_date "<<split_date<<endl;
    // if (job->nb_requested_resources > _nb_machines) 
    // {
    //     _decision->add_reject_job(job->id, split_date);
    // }
    // cerr<<"reject sucessfully"<<endl;

    if (assert_insertion_successful)
        PPK_ASSERT_ERROR(false, "Error in Schedule::add_job_first_fit: could not add job '%s' maybe there are not enough processors", job->id.c_str());
       //     to_string().c_str()); 
...
void Schedule::remove_job_internal(const Job *job, Schedule::TimeSliceIterator removal_point)
{
    ...
    // remove failed_machines from the job_machines, 
    // job_machines are the processors the batsim want to bring back (the processors using by finished jobs)
    for (auto it = _failed_machines.begin(); it != _failed_machines.end(); it++) {
        job_machines.remove(*it);
        // cerr<<"For job "<<job->id<< ", remove processor "<<*it<<" from its job_machines"<<endl;
        
        // cerr<<"Now the job_machines contains: ";
        // for (auto it = job_machines.elements_begin(); it != job_machines.elements_end(); ++it) {  
        //         cerr << *it <<" ";
        // }
        // cerr<<endl;
    }
    ...
    for (auto pit = removal_point; pit != _profile.end(); ++pit)
    {
        ...
        if (pit->allocated_jobs.erase(job) == 1)
        {
            ...
            pit->nb_available_machines += job_machines.size();//job->nb_requested_resources; //is the res in the original workload 
            //cerr<<"job_machines size: "<<job_machines.size()<<" nb_requested_resources: "<<job->nb_requested_resources<<endl;
            ...
            if (pit != _profile.begin())
            {
                ...
                {
                    if (previous->available_machines == pit->available_machines) {
