...
{
    const set<string> variants_set = {"conservative_bf", "conservative_bf_node_stealing", "my_scheduler", "crasher", "easy_bf", "easy_bf_fast",
    ...
    //cerr<<"socket_endpoint "<<socket_endpoint<<endl;
        ...
        for (unsigned int event_i = 0; event_i < events_array.Size(); ++event_i)
        {
            ...
            else if (event_type == "JOB_SUBMITTED")
            {
                ...
                if(scheduling_variant == "conservative_bf_node_stealing")
                {
                    ...
                    //~ int test = event_data["job"]["data"].GetInt(); /* Maxime */
                    //~ rapidjson::GenericValue<rapidjson::UTF8<> >::ConstArray tab_data = event_data["job"]["data"].GetArray(); /* Test */
                    //~ auto tab_data = event_data["job"]["data"].GetArray(); /* Test */
                    ...
                    double Tfirst = 0.0;
                    int pos1 = job_id.find("-"); //exclude killed job copy like w0!1-1
                    int pos2 = job_id.find("_"); //exclude checkpoint job like resubmit!1_2-1_checkpoint
                    if (pos1 == -1 && pos2 == -1)
                    {
