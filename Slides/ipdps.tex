...
\begin{figure}[tb]
		...
		\begin{tikzpicture}[thick]
			...
			%~ (1) edge[bend right=90] node [left] {} (2); 
			 
			\onslide<6-12> { \draw[color=black, fill=blue!40] (-2.9*\x, 1.5*\y) rectangle (1.9*\x, 1.4*\y)  node[midway] (5) {}; }
			
			\onslide<5-6> { \path[every node/.style={font=\sffamily\small}] (4) edge[bend right=90] node [left] {} (5); }
			
			\onslide<7-7> { \draw[color=black, fill=blue!40] (-2.9*\x, 1.7*\y) rectangle (1.9*\x, 1.6*\y) node[midway] (6) {}; }
			
			\onslide<6-7> { \path[every node/.style={font=\sffamily\small}] (3) edge[bend right=90] node [left] {} (6); }
			
			\onslide<8-10> { \draw[color=black, fill=blue!40] (-2.9*\x, 0.8*\y) rectangle (1.9*\x, 0.7*\y) node[midway] (7) {}; }
			
			\onslide<7-8> { \path[every node/.style={font=\sffamily\small}] (6) edge[bend right=90] node [left] {} (7); }
			
			\onslide<10-12> { \draw[color=black, fill=blue!40] (3.1*\x, 3.7*\y) rectangle (7.9*\x, 3.6*\y)  node[midway] {}; }
			\onslide<11-> { \draw[color=black, fill=blue!40] (3.1*\x, 3.9*\y) rectangle (7.9*\x, 3.8*\y)  node[midway] {}; }
			\onslide<12-13> { \draw[color=black, fill=blue!40] (3.1*\x, 1.5*\y) rectangle (7.9*\x, 1.4*\y)  node[midway] {}; }
			\onslide<13-13> { \draw[color=black, fill=blue!40] (3.1*\x, 0.8*\y) rectangle (7.9*\x, 0.7*\y)  node[midway] {}; }
			\onslide<13-> { \draw[color=black, fill=blue!40] (-2.9*\x, 0.8*\y) rectangle (1.9*\x, 0.7*\y)  node[midway] {}; }
		\end{tikzpicture}
		}
\end{figure}
\end{frame}
...
\begin{frame}{Eviction and optimizations}
	\begin{alertblock}{Our eviction policy: LUF (Least Used in the Future)}
		...
		% \end{itemize}
		\begin{enumerate}
			\item If possible, try to evict data not useful for any task in \pulledTasks{k} and used by a minimal number of tasks in $\plannedTasks{k}$
...
\begin{block}{Limiting data movements is crucial to extract the best performance out of GPUs}
	...
	\emph{New strategy: DARTS, focused on data locality}
\end{block}
