...
We also noticed users genome alignment tools that require to store text large files, and it is likely that they need to load multiple of them, resulting in high I/O demands. 
...
we randomly selected\footnote{We used the random from python3.6.9 with random.seed(0), nums = list(range(1, 52)) and random.shuffle(nums) which produce the order [28, 13, 43, 41, 42, 8, 6, 36, 2, 50, 40, 1] for the first twelve numbers, that matches the week number on the calendar.} \todo[inline]{LM: the footnote is not necessary, these details need to go in the artifact only}
12 weeks and
...
on a simulator that we developed\footnote{Code and anonymized logs are available for reproducibility:  \url{https://github.com/userdoubleblind/Locality-aware-batch-scheduling}}.
