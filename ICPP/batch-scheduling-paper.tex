...
To perform such identification, the Kraken2 tool\footnote{See \url{https://github.com/DerrickWood/kraken2/tree/master/src}} is often used together with an input database, whose size vary from a few GB up to 800GB.
We also noticed the use of genome alignment tools that require to store text large files, and it is likely that they need to load multiple of them, resulting in high I/O demands.
...
\todo[inline]{LM: the footnote is not necessary, these details need to go in the artifact only. Max Okay I removed it and updated the footnote of the artifact if it's ok ?}
12 weeks and
...
We evaluate the proposed scheduler on these metrics and compared them to FCFS and EFT, with and without backfilling.
...
The simulator is a simple event-based simulator focusing on data locality. It follows these principles:
is made in python and follows these principles:
\begin{itemize}
...
\item When a job is submitted, the scheduler  knows its submission time, requested walltime, number of cores required and input file name of the jobs;
\item Scheduling a job consists in providing to the job a start time, a node, and as many cores on that node as requested.
\end{itemize}
...
\rev{Among the 12 randomly selected weeks, different workload conditions arise. We first concentrate on a low utilization.
...
In other words we plot $\mathit{Stretch\_FCFS}/\mathit{Stretch\_Scheduler}$ for each \us and each scheduler.
...
these \uss are depicted on this figure, for the sake of clarity. 
The solid line is the median and the triangles represent mean
improvement. Because outliers bring a huge improvement, the means are urually higher than the medians.
...
times larger with the scheduler than with to FCFS.
For all boxplots shown in this paper, the box contains results within
the [25\%,75\%] range while whiskers extend to 12.5\% and 87.5\%.
...
results in almost no improvement, nor degradation of the \us stretch,
apart from LEA that degrades the performance more than 50\% of it's results.
}
...
\todo[inline]{LM: the previous sentence is obscure to me}
\begin{figure}[t]\centering\includegraphics[width=0.9\linewidth]{../MBSS/plot/Stretch_times/Stretch_times_FCFS_LEA_2022-10-03->2022-10-09_V10000_anonymous_450_128_32_256_4_1024.png}\caption{Stretch times of each user session from week 40 using LEA compared to FCFS.}\label{40_fcfs_vs_lea}\end{figure}
\begin{figure}[t]\centering\includegraphics[width=0.9\linewidth]{../MBSS/plot/Stretch_times/Stretch_times_FCFS_LEM_2022-10-03->2022-10-09_V10000_anonymous_450_128_32_256_4_1024.png}\caption{Stretch times of each user session from week 40 using LEM compared to FCFS.}\label{40_fcfs_vs_lem}\end{figure}
...
\todo[inline]{LM: I think figure  Figure~\ref{40_fcfs_vs_lea} and  Figure~\ref{40_fcfs_vs_lem} should use a log scale on the Y axes, to that 2 and 0.5 (twice better or twice worse) is at the same distance from 1 (same for Figure~\ref{vs_fcfs_lea_43})}
...
on the queue times.
\rev{
\begin{figure}[t]\centering\includegraphics[width=0.9\linewidth]{../MBSS/plot/Boxplot/byuser/box_plot_stretch_10-24-10-30_1.pdf}\caption{Stretch's improvement from FCFS on week 43}\label{stretch.43}\end{figure}
\begin{table}
\begin{tabular}{ l c >{\columncolor{green!85}}c c c }
  \toprule
  Schedulers & EFT & LEA & LEO & LEM \\
...
\begin{figure}[t]\centering\includegraphics[width=0.9\linewidth]{../MBSS/plot/Stretch_times/Stretch_times_FCFS_LEA_2022-10-24->2022-10-30_V10000_anonymous_450_128_32_256_4_1024.png}\caption{Stretch times of each user session from week 43 using LEA compared to FCFS.}\label{vs_fcfs_lea_43}\end{figure}
...
\begin{figure}[t]\centering\includegraphics[width=0.9\linewidth]{../MBSS/plot/Boxplot/byuser/box_plot_stretch_all-all_1.pdf}\caption{Stretch's improvement from FCFS on all evaluated weeks.}\label{boxplot.all}\end{figure}
...
\caption{Percentage of reduction from the amount of time spent 
waiting for a file to be ready before starting the computation,
relative to the total waiting time of FCFS on all workloads.\todo[inline]{Max: Do we want to put green on the best one? It's always LEA on our tables.\\LM: put in in bold face, it is sufficient and quite common.}}\label{tab.all}
\end{table}
...
EFT is not able to see that a large number of jobs using the same file should be scheduled on the same node: even if it generates more queue time, the overall execution time will be lowered thanks to data re-use.
...
Thus, \rev{LEA} is the strategy that least to more significant improvements: it is able to compute jobs between~1 and~2 times faster in 50\% of the cases and between~2 and~6 times faster in 25\%.
...
\todo[inline]{LM: we should conclude here by saying something better about LEM, saying it is more versatile, right ?}

\paragraph{\rev{Results with backfilling}}
\begin{figure}[t]\centering\includegraphics[width=0.9\linewidth]{../MBSS/plot/Boxplot/byuser/box_plot_bf_stretch_all-all_1.pdf}\caption{Stretch's improvement from FCFS with backfilling on all evaluated weeks.}\label{boxplot.all_bf}\end{figure}
...
\todo[inline]{LM: also give some figure about median improvement here: 7\% ?}
