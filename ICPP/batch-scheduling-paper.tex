...
EFT takes into account file transfers when scheduling but is a less agressive strategy, the economy in data transfers is only 0.9\% (see Table~\ref{tab.all}).
Indeed, it's not able to see that a large number of jobs suinsg the same file should be scheduled on the same node: even if it generate more queue time, he overall execution time will be lowered thanks to data re-use.
