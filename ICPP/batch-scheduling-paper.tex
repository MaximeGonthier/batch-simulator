...
As mentionned above, we observed behavior of IO intensive jobs on the platform we extracted the logs from. The cluster is often used
...
There have been 1083 distinct users and 136\,404 user's sessions.
...
We evaluated our schedulers on 12 randomly selected weeks offering different workload conditions.
First, we study the behavior on an underloaded cluster.
}
...
An improvement above 1 is a speed-up. An improvement at 0.5 means that the sum of the queue times and transfer times for this user's session was two times bigger with the scheduler compared to FCFS. At 1.5 this sum was 1.5 times smaller compared to FCFS.
...
If no percentages are written, it means that all the user's sessions have been plotted.
...
on the queue times.
\rev{
\begin{figure}[t]\centering\includegraphics[width=0.9\linewidth]{../MBSS/plot/Boxplot/byuser/box_plot_stretch_10-24-10-30_1.pdf}\caption{-}\label{stretch.43}\end{figure}
\begin{table}
\begin{tabular}{ |c|c|c|c|c| }
  \hline
  Schedulers & EFT & LEA & LEO & LEM \\
...
Figure~\ref{tab_core.all} shows the reduction in terms of total core time (i.e. the core time on each core of each node over the 12 weeks). 
