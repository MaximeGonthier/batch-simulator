...
\newcommand{\us}{\ensuremath{\mathit{user's~session}}\xspace}
\renewcommand{\algorithmicrequire}{\textbf{Input:}}
\renewcommand{\algorithmicensure}{\textbf{Output:}}

\begin{document}

\title{Locality-aware batch scheduling of I/O intensive workloads\todo[inline]{Max: Same title?}}
...
\todo[inline]{Max: Can we keep the same abstract?}
...
\todo[inline]{Max: Do we want, from the introduction, to talk more about the application cases that needs large input data?}
...
\begin{itemize}
	...
	\item We implement all three heuristics as well as two state-of-the-art schedulers on a simulator and study the performance (mean stretch, amount of time spent waiting for a file to be loaded \rev{and number of core hours}\todo[inline]{Max: Do we say "core hours" or it must be more explicit?}) obtained \rev{after scheduling almost 2 millions jobs} (Section~\ref{sec.evaluations}).
...
\todo[inline]{Max: I define \us here (do we want a macro like that for it?):}
\rev{
Finally, what is important for a user is that all the jobs he has submitted at once
are finished as soon as possible. Thus we introduce the notion of \us. A \us is the sum of the stretch of jobs submitted by a user over a short period of time. It is the stretch of each \us that will be used in the Figures of Section~\ref{sec.evaluations}.
}
...
\rev{We observed such behavior on the platform we extracted the logs from. The cluster is often used
for taxonomic identification of DNA fragments. To perform such identification, the Kraken2 tool\footnote{See \url{https://github.com/DerrickWood/kraken2/tree/master/src}} is often used together with an input database of multiple GB\footnote{See \url{https://benlangmead.github.io/aws-indexes/k2} for example}. Those database vary in size, from a few GB up to 800GB.
We also observed users using the Hierarchical alignment tool for genome alignment \footnote{See \url{https://github.com/ComparativeGenomicsToolkit/hal}} that uses as an input MAF files. MAF is a text format used to store genome alignment
\footnote{For example for primates you have: \url{https://ftp.ensembl.org/pub/release-107/maf/ensembl-compara/multiple_alignments/10_primates.epo/}} and users might need to load multiple of them to work on their project, resulting in high IO demands.
}
%
...
\rev{Two users could use the same file but it is very unlikely. Since users are using subsets 
of large multi-GB databases and are subitting jobs at different times among thousands of other jobs, 
it is very unlikely that two users using the same file submit jobs at the same time.}
...
\rev{Indeed, if a user needs 128GB of memory for example, but only requests 1 core out of the 20 available, he is blocking the machine for all the other users. Consequently, it's more efficient for a user who needs all the memory of a machine to reserves all the cores of the machine. And similarly if a user needs a fraction of the memory, he can reserves a fraction of the machine's cores. 
A node can thus contain multiple different files. Thus the scheduling strategy cannot be 
simplified to assign a node per user's session because the scheduler must maximize locality while avoiding starvation.}
...
\caption{\rev{Methodology followed to schedule and evaluate jobs from a specific week while avoiding edge effects.}}\label{fig.workload}
\label{fig.ex}
\end{figure}
...
To target different scenarios and avoid scheduling the whole year, 
we randomly selected\footnote{We used the random from python3.6.9 with random.seed(0), nums = list(range(1, 52)) and random.shuffle(nums) which produce the order [28, 13, 43, 41, 42, 8, 6, 36, 2, 50, 40, 1] for the first twelve numbers, which corresponds to the weeks' number on the calendar.} 12 weeks and
extract the jobs corresponding to days within these weeks from the logs. 
Over these 12 weeks of workloads, we scheduled 1986496 jobs and evaluated 1493151 of them.
There have been 1083 distinct users and 136404 user's sessions.
...
We evaluate the stretch of each user's session, the total amount of time spent waiting for a file to be available as well as the total core time.
}
...
\begin{itemize}
	...
	\item The jobs are scheduled on the cores that will minimize the creation of "holes" on the node \todo[inline]{Max: Not clear, how should I formulate this?}
	\item The scheduler only knows the jobs whose submission time are superior or equal to the current time
	\item The scheduler only knows the subssion time, requested walltime, number of cores required and input file name of the jobs
\end{itemize}

We didn't use an existing simulator of batch systems like Batsim because it was missing some 
key elements for us, notably, the possibility to add a loading time or the input files before the start of a job or
the possibility to re-use an input file already loaded on a node to skip this loading time.
}

\subsection{\rev{Performance on an underutilized cluster}}\label{sec.40}
\todo[inline]{Max: I have to present each fig here before explaining, it might be tiring for the reader.}
...
under-loaded cluster.
Figure~\ref{stretch.40} depicts 
\rev{
the distribution of each user's sessions' stretch's improvement compared to FCFS obtained by each strategy on week 40.
In other words it's for each user's session and for each scheduler: $\frac{Stretch\_FCFS}{Stretch\_Scheduler}$.
In this figure, the horizontal black dotted line corresponds to no improvements from FCFS, which
is the situation where the sum of the queue time and transfer time of the user's session is the same as with FCFS.
The solid line is the median and the triangle symbolizes the mean improvement.
An improvement above 1 is a speed-up. An improvement at 0.5 means that the sum of the queue times and transfer times for this user's session was two times bigger with the scheduler compared to FCFS. An improvement at 1.5 means that the sum of the queue times and transfer times for this user's session was 1.5 times smaller with the scheduler compared to FCFS.
All the boxplot shown in this paper use octiles to draw the whiskers. 
The percentages next to the scheduler's names are the proportion of user's session
where the improvement was different from 1 by at least 1\%. We only plot those user's sessions.
If no pecentages are written, it means that all the user's sessions have been plotted.
...
The consequence for the stretch is immediately distinguishable on Figure~\ref{40_fcfs_vs_lea}.
This visualization shows for each \rev{user's session}, the ratio of its stretch with LEA by its stretch with FCFS (hence a value above~1 means LEA improves the stretch).
...
\rev{
Figure~\ref{tab.40} shows the reduction from the amount of time spent 
waiting for a file to be ready before starting the computation,
...
to their flexibility, achieve performance identical to EFT.
...
\caption{Percentage of reduction from the amount of time spent 
waiting for a file to be ready before starting the computation,
relative to the total waiting time of FCFS on week 43.\todo[inline]{Max: Do I put "-" before ?}}\label{tab.43}
\end{table}
\begin{figure}[t]\centering\includegraphics[width=0.9\linewidth]{../MBSS/plot/Stretch_times/Stretch_times_FCFS_LEA_2022-10-24->2022-10-30_V10000_anonymous_450_128_32_256_4_1024.png}\caption{Stretch times of each job of LEA compared to FCFS on the workload of week 43.}\label{vs_fcfs_lea_43}\end{figure}
\todo[inline]{Max: Not sure we want to keep this figure}
...
on the queue times.
\rev{
As you can see on Figure~\ref{tab.43}, LEA and LEM greatly reduces the transfer time.
}
...
In this case, LEA's strategy, also found in LEM, allows to greatly reduce the stretch of each user's session.
We can observe this, job by job, on Figure~\ref{vs_fcfs_lea_43}: very few jobs 
have a worse stretch than FCFS and a large amount of jobs are above an improvement \rev{above} 2.


\subsection{\rev{Aggregated results on 12 different evaluated weeks of the workload}\todo[inline]{Max: Maybe another title would be better?}}
...
\caption{Percentage of reduction from the amount of time spent 
waiting for a file to be ready before starting the computation,
relative to the total waiting time of FCFS on all workloads.\todo[inline]{Max: On veut du vert ? Ce sera toujours LEA}}\label{tab.all}
\end{table}
\begin{table}
\begin{tabular}{ l c >{\columncolor{green!85}}c c c }
  \toprule
  ...
  \bottomrule
\end{tabular}
\caption{Percentage of reduction of the total core time used on all workloads.\todo[inline]{Max: La la précision après la virgule n'est pas la même}}\label{tab_core.all}
\end{table}
...
on heavily saturated clusters (see Section~\ref{sec.43}).
In these cases, the \rev{improvements} of LEA and LEM are much higher than those of FCFS or EFT.
...
However 87.5\% of its results are above 1, i.e., an improvement, whereas for LEA, only approximately 75\% of the results are above 1. 
...
\rev{
Figure~\ref{tab.all} shows us that LEA gretaly reduce data transfers, as well as LEM.
LEO behave similarly to EFT.
Figure~\ref{tab_core.all} shows the reduction in terms of total core time (i.e. the core time on each core of each node over the 12 weeks). LEA greatly reduces the total core time used, this reduction corresponds to 49000 core hours, which corresponds to a reduction of 5 hours on each 20 core of the 486 nodes. This is a significant reduction in the cluster's utilization.
}
...
strategy is much more consistent, while still having great improvements in some cases, \rev{as can be seen with mean value}.
