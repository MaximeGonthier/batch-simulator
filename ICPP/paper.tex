...
for taxonomic identification of DNA fragments. To perform such identification, the kraken2 tool \url{https://github.com/DerrickWood/kraken2/tree/master/src} is often used together with an input database of multiple gygabytes (see \url{https://benlangmead.github.io/aws-indexes/k2}. Those data base vary in size, 
from a few gigabyte up to 800GB.
We also observed users using the Hierarcchicakl aligmenet tool for genome alignement (see \url{https://github.com/ComparativeGenomicsToolkit/hal}) that uses databases of MAF is a text format used at UCSC to store genome alignment (see \url{https://ftp.ensembl.org/pub/release-107/maf/ensembl-compara/multiple_alignments/10_primates.epo/} to download multiple genome files). 
}
...
\todo[inline]{Reviewer 1: "Does it also give number of required threads or will nb threads be dynamically decided based on available cores - if so, the runtime on different machines will be different"}
...
\rev{A node can thus contain multiple different files. Thus the scheduling strategy cannot be 
simplified to assign a node per user's session. Moreover, you must maximize locality while avoiding starvation.}
\todo[inline]{Reviewer 1: "Is there a reference that demonstrates that file size is directly proportional to processing time for HPC applications?" I think he meant number of cores. Dire que si t'as 1 cores et 128 gb tu bloque les 19 autres coeurs de toute facon donc autant en prendre un chiffre proportionel: a voir si on le garde car pas forcément utile}
...
\caption{\rev{Methodology followed to schedule and evaluate jobs from a specific day while avoiding edge effects.}}\label{fig.workload}
\label{fig.ex}
\end{figure}
...
on a simulator that we developed.
En python, mesure ca et ca et pas basé je recalcule, Pk pas utilisé un truc qui exsite ? Car fonctionalité manquante 
\todo[inline]{Put url for simulator here and modiy paragraph about it earlier in the section.}
}
...
\begin{figure}[t]\centering\includegraphics[width=0.9\linewidth]{../MBSS/plot/Stretch_times/Stretch_times_FCFS_LEA_2022-10-03->2022-10-09_V10000_anonymous_450_128_32_256_4_1024.png}\caption{With LEA.}\label{07_16_fcfs_vs_lea}\end{figure}
\begin{figure}[t]\centering\includegraphics[width=0.9\linewidth]{../MBSS/plot/Stretch_times/Stretch_times_FCFS_LEM_2022-10-03->2022-10-09_V10000_anonymous_450_128_32_256_4_1024.png}\caption{With LEM.}\label{07_16_fcfs_vs_lem}\end{figure}
