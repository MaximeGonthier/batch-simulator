...
Thus, on underused clusters, LEO will have a behavior close to EFT, while still trying to minimize evictions.
...
Here we evaluate our strategies on an almost saturated cluster that underline the weakness of LEM.
...
enough jobs to cover all nodes, thus decreasing largely the time spent waiting for a file
and consequently reducing the stretch of each job.
Compared to LEA, LEM has a smaller mean value but 75\% of the results are above 1.
Whereas for LEA, only approximately 57\% of the results are above 1. 
Being a mixed strategy is beneficial to LEM and offers us more sustained performance at the cost
of fewer extreme speed-up.
For LEO, 87,5\% of the results are above a speed-up of 1, which shows that our opportunistic
strategy is much more consistent.
...
with backfilling (hashed bars), the mean stretch is much lower. 
However our strategies do not benefit as much as FCFS with backfilling for two reasons:
\begin{itemize}
	\item Even if we consider data locality when backfilling, trying to fill as much as possible a node and optimizing data re-use are two contrary goals. The second one want to create an order that nicely re use data while the first one just cares about filling holes. So when backfilling, we sometimes compromise a nice re-use pattern planned by LEA.
	\item Our strategies, especially LEO, are already able to nicely fill the nodes without needing backfilling. Indeed, grouping jobs by input file allow to have similar jobs on the same nodes. Jobs having the same length and number of requested cores are much easier to use to fill a node than a completely heterogeneous set of jobs. Consequently, the shortfall without backfilling is much lower for LEA, LEO and LEM than for FCFS and EFT.
\end{itemize}
Thus, we still reduce queue time with backfilling, but the gain of our strtagey is much lower.
On Figure~\ref{boxplot.all_bf}, for both EFT and LEO, the speed-ups are not significant.
LEA has a median slightly worse than our competitor. However it's still have a very high mean.
