...
represented in Figure~\ref{fig.workload}).\todo[inline]{Elisabeth: I think that in principle this paragraph is enough together with the picture. We need to clarify that we consider jobs submitted on a specific day rather than jobs running a specific day. Suggestion: In order to avoid simulating the whole workload (one year) but to
target different scenarios, we randomly select a number of days (44 days) and
