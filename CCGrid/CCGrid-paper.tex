...
\todo[inline]{LM:do we have jobs without input files?}
...
that file sizes are multiple of $M/m$.  \todo[inline]{LM: I wonder if
  this constraint should be here or in the experimental setting? I
  think that the proposed algorithms are general enough to handle
  cases where jobs have any size.}
...
be evicted.  \todo[inline]{LM: I thought we removed this constraint,
  and replaced it by a weaker one: a file may be evicted if some
  memory is required by another job between $J_i$ and $J_j$. Correct?
  Do we implement this for all strategies?}
