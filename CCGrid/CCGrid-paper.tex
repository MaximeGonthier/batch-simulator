...
%~ Abstract by Loris:
  ...
  SLURM \todo[inline]{Max: Should we explicit the "Simple Linux Utility for Resource Management"?LM: I think it's fine in the abstract, we give a reference below. Max: Okay.} or OAR to allocate computing jobs onto nodes. These schedulers
  ...
  larger job, more difficult to schedule by the resource manager, and would
  ...
  computing nodes, provided it does not impact the storage needs of
  ...
  the computing nodes as well as being aware of what the storage of each
  node contains in order to re-use an input file
  already loaded in the storage of some node as many times as possible.
  ...
  The results show that keeping data in local storage between successive
