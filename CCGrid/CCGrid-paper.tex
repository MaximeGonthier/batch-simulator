...
a score to choose the node that optimizes file loads (section~\ref{subsec.lea}) and others
are opting for a mixed strategy between locality and earliest possible start time
(sections~\ref{subsec.leo} and~\ref{subsec.lem}).
...
\todo[inline]{M&x: Talk about cores in the framework?}
...
\subsection{A locality-focused algorithm: LEA}\label{subsec.lea}
...
\subsection{LEO}\label{subsec.leo}
...
\subsection{LEM}\label{subsec.lem}
...
Additionally, when backfilling, the available cores with the smallest next 
...
Since explicit data dependencies are not encoded in SLURM job specifications, we created an artificial pattern for this that replicate the behavior of an user.
...
\begin{enumerate}
	...
	\item $J_i$ and $J_j$ were submitted in a 800 seconds timeframe. We consider this timeframe to be a reasonable amount of time for a user to submit all of his jobs using the same input file.
\end{enumerate}
...
Having $L = {l_1, l_2, ..., l_n}$ different nodes sizes raises a new constraint. The model would have
$L$ subsets of nodes, each possibly containing a different number of nodes.
Some input files would be to large to be computed on one or multiple of those subsets.
Thus, in addition to the scheduling problem, a load balancing problem arises.
For example, should a job using a file of size $l_1$ be scheduled on a node of size $l_2$?
Indeed, the scheduler need to be able to manage each subset of nodes as an independent 
resource that need to avoid starvation while also not being saturated
in case of a sudden batch of jobs requesting a node of it's size.
...
Each application scenario evaluated in the next section is a day on the log's history of the workload.
The way we collect and schedule jobs from the log's history is represented on Figure~\ref{fig.workload}.
\begin{figure*}[tb]
...
i.e. in the situation in which it was on the real cluster while also to avoiding any side effect.
...
This allows us to exactly replicate the state of the cluster on this day before
scheduling jobs from \textit{Day 0}.
...
Our main competitors on this measures will be FCFS and EFT, with and without backfilling.
...
\begin{figure}[H]\centering\includegraphics[scale=0.41]{../MBSS/plot/Cluster_usage/2022-07-16->2022-07-16_V10000_Fcfs_Used_nodes_Reduced_450_128_32_256_4_1024_core_by_core.pdf}\caption{Visualization of the utilization rate of the cluster on the workload of July the 16th with FCFS.}\label{07_16_cluster_usage_fcfs}\end{figure}
\begin{figure}[H]\centering\includegraphics[scale=0.41]{../MBSS/plot/Cluster_usage/2022-07-16->2022-07-16_V10000_Fcfs_with_a_score_x500_x1_x0_x0_Used_nodes_Reduced_450_128_32_256_4_1024_core_by_core.pdf}\caption{Visualization of the utilization rate of the cluster on the workload of July the 16th with LEA.}\label{07_16_cluster_usage_lea}\end{figure}
...
that we evaluate.
The gray line is the number of cores currently loading a file.
...
that heavily weigh the transfer time \todo[inline]{Max: We use $t_k$ as the "earliest available time to compute x cores on node k" in the schedulers section. Can we use it here?} or weigh equally $t_k$, the transfer and eviction time. 
Thus, on underused clusters, LEO will have a behaviour\todo[inline]{Max: Or behavior? It's british or american english right?} close to EFT, while still trying to minimize evictions.
Similarly, LEM switches between EFT and LEA depending on the cluster's usage.
Here the cluster's usage is under 80\%, so LEM behave similarly to EFT on this particular workload.
On the contrary, LEA favours\todo[inline]{Max: Or favor?} data re-use over an early start time for a job.
...
Jobs in this queue are jobs that already have a valid copy of their file loaded on a node. 
...
The consequence in term of stretch is immediately noticeable on Figure~\ref{07_16_fcfs_vs_lea}. This visualization shows the stretch's speed-up of each job compared to FCFS. The size of a circle is proportional to the job's duration. On the workload of July 16, we can observe, below 1, columns of jobs submitted at the same time and with the same duration (so probably using the same file), meaning LEA was waiting to re-use the file to start the jobs, leading in a larger queue time when other nodes are available.
...
\end{subfigure}
\caption{Visualization of the utilization rate of the cluster on the workload of September the 09th.\todo[inline]{Max: Is it readable to have this two figures side by side?}}\end{figure}
...
And as we saw in the last section, it's a sub-optimal heuristics when the cluster is not fully saturated.
...
\begin{figure}[H]\centering\includegraphics[scale=0.41]{../MBSS/plot/Cluster_usage/2022-03-26->2022-03-26_V10000_Fcfs_Used_nodes_Reduced_450_128_32_256_4_1024_core_by_core.pdf}\caption{Visualization of the utilization rate of the cluster on the workload of March the 26th with FCFS.}
\label{cluster_usage.03-26_fcfs}\end{figure}
\begin{figure}[H]\centering\includegraphics[scale=0.41]{../MBSS/plot/Stretch_times/Stretch_times_FCFS_LEM_2022-03-26->2022-03-26_V10000_450_128_32_256_4_1024.pdf}\caption{Stretch times of each job of LEM compared to FCFS on the workload of March the 26th.}
...
On a saturated cluster, filling all the cores with the first jobs of the queue is not crucial.
It is more beneficial to group jobs using the same file.
The first few jobs will have a longer queue than with FCFS,
but, overtime, re-using files cause a snowball effect that will reduce the 
...
Thus, LEA's strategy, also found in LEM, allows to greatly reduce the mean stretch.
...
\begin{figure}[tb]\centering\includegraphics[scale=0.41]{../MBSS/plot/Results_FCFS_Score_Backfill_2022-08-16->2022-08-16_V10000_Mean_Stretch_450_128_32_256_4_1024.pdf}\caption{Average stretch of all jobs evaluated on August 16.}
\label{stretch.08-16}\end{figure}
\begin{figure}[H]\centering\includegraphics[scale=0.41]{../MBSS/plot/Cluster_usage/2022-08-16->2022-08-16_V10000_Fcfs_Used_nodes_Reduced_450_128_32_256_4_1024_core_by_core.pdf}\caption{Visualization of the utilization rate of the cluster on the workload of August 16 with FCFS.}
...
it's competitors. LEM is almost 7 times smaller. This drastic diminution is explained
...
Thus, reducing the transfer times of small jobs has a much greater effect on the stretch, resulting in this 
...
results of the stretch's speed-up compared to FCFS over all 44 evaluated workloads.
...
We can explain this huge increase from the good performance of LEA's strategy
...
For LEO, 87,5\% of the results are above a speed-up of 1, which shows that our opportunistic
...
Figure~\ref{boxplot.all_bf} shows the results but with the backfilling version of our
schedulers and compared to FCFS with backfilling over all workloads.
...
with backfilling (hashed bars), the mean stretch is much lower for FCFS and EFT. 
However, our strategies do not benefit from backfilling as much as FCFS does for two reasons:
\begin{itemize}
	...
	Consequently, the shortfall without backfilling is much lower for LEA, LEO and LEM than for FCFS and EFT.
\end{itemize}
...
On Figure~\ref{boxplot.all_bf}, for both EFT and LEO, the speed-ups are not significant.
LEA has a median slightly worse than our competitor. However it's still have a better mean.
LEM has more than 50\% of it's results that are better than FCFS-BF, with a mean at 1.31.

\todo[inline]{LM: should we say somewhere that we tested with stronger data persistence and it did not really helps?}

\section{Conclusion}\label{sec.conclusion}

\todo[inline]{Max: - Fairness - Advance reservation - Tune LEM - Improve locality of LEO - Tune BF for LEA, LEO and LEM}
