...
This work is aimed at the Map-Reduce model and allow to predict future jobs arrivals, two prerequisites that we do not consider.
...
First-Come-First-Serve or FCFS, detailled on
Algorithm~\ref{algo.fcfs}. Implementing this scheduler requires to
remember for each core the time at which it will become
...
As mentionned above, backfilling has been proposed to increase the
performance of job schedulers, by allowing jobs with low priority to be
scheduled before jobs with higher priority. In order to avoid jobs
being perpetually delayed, bounds have to be set on how already
scheduled jobs can be affected by backfilling. As discussed above,
...
For each of the previous scheduling strategy, we consider a variant
