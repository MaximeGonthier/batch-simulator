...
\todo[inline]{Details on: data intensive applications, challenges and objective}
...
\begin{itemize}
	...
	\item We extract jobs information from historical logs of a cluster to build workloads that corresponds to the needs and behaviors of real users (Section~\ref{sec.working}).
	...
	Our evaluation demonstrate that our heuristics surpasses in most cases the state of the art schedulers.
...
To schedule jobs on batch systems, systems like:
the Slurm Workload Manager~\cite{SLURM}, OAR~\cite{oar} or TORQUE~\cite{torque} are prominent.
Slurm is used on most HPC clusters. Its default strategy is to schedule jobs based on priorities derived
from submission time, in other words a First-Come First-Serve strategy~\cite{slurm_website_scheduling}.
%~ Slurm is used by a number of production level clusters such as PlaFRIM~\cite{plafrim}, etc..
On top of that, a backfilling algorithm is often used~\cite{New_Backfill}.
\todo[inline]{Max: We talk about and cite backfilling in the scheduler section. Should we also 
do it here? Or only here? Or only in the scheduler section?}
...
To optimize the utilisation rate of a cluster, many strategies have been proposed like
Gang scheduling~\cite{gang_scheduling} or Maui~\cite{Maui_Scheduler}.
However these schedulers do not consider the issue of dealing with data-intensive workloads.
...
%~ They use detailed knowledge of the workload characteristics.
...
By not using HDFS or any distributed file system, we avoid altogether these problems. 
...
We present here various schedulers used to allocate jobs to
...
\todo[inline]{should be we present backfilling here or in the introduction?}

\todo[inline]{Max: Do we need to talk about the objective in the framework?}
...
\todo[inline]{LM: move following paragraph to last subsection}
...
The previous strategies focus on starting as soon as possible (FCFS)
of finishing as soon as possible a job (EFT).  Those are good methods
to avoid starvation of a node and reduce queue times.  However, they
may lead to loading the same input file on a large number of nodes in
the platform, simply to minimize immediate queue times. Time is spent
loading the input file multiple times and this can affect the global
performance of the system by delaying all jobs.  We present here three
strategies that attempt to better take into account data locality in
order to reduce queuing times in the long run by increasing data reuse.
...
\begin{itemize}
\item The available time for computation $t_k$;
...
in the memory requires to evict many other files, these file will not
...
at the availability time is usually much larger than the time for
...
The LEA strategy gives a dramatic importance on data loads. Hence, it
	...
	\ForEach{$J_i \in$ the job's queue}
		\ForEach{$\Node{k} \in \nodeset$}
			...
			\State $\mathit{LoadOverhead} \gets t_k' - t_k$ %\Comment{It thus gets the load's duration}
                        \State Let $\mathcal{F}$ be the set of files in the memory of node \Node{k} at time $t_k$
			\State $\mathit{EvictionPenalty} \gets (\sum_{F_j\in\mathcal{F}}\size(F_j) \times \size(\file(J_i))/\memory(\Node{k}))/\bandwidth$
			\If{$t_k = \mathit{current_time}$}
				\State $score_{\Node{k}} \gets 't_k$
			\Else
			         \State $score_{\Node{k}} \gets t_k + W \times \mathit{LoadOverhead} + \mathit{EvictionPenalty}$
			\EndIf
		\EndFor
...
LEM but performs a simple mix between the EFT and the LEA strategies:
...
otherwise we the EFT strategy is used.
...
[ font=\footnotesize,
  ...
  extra y ticks={0,1,2,3,4,5},
  extra x ticks={0,1,2,3,4,5},
  extra y tick style={yticklabel={$C_{\pgfmathprintnumber{\tick}}$}}] 
\addplot[fill=orange!20] coordinates {(0,0) (0,3) (2,3) (2,0)} node at (current path bounding box.center) {Job 1}; 
\addplot[fill=red!20] coordinates {(2,0) (2,5) (3,5) (3,0)} node at (current path bounding box.center) {Job 2}; 
\addplot[fill=blue!20] coordinates {(3,0) (3,2) (5,2) (5,0)} node at (current path bounding box.center) {Job 3};
...
start time of a job are chosen, in order to fill as much as possible the nodes."}
...
Each application scenario evaluated in the next section is one day of the log's history of the workload.
...
a simulator that we developed\todo[inline]{Max: Do we need to say more here? Elisabeth: Citation to framework used for simulator or did you in the end make it from scratch?
Max: Yes I made it from scratch.}.
...
Thus, on underused clusters, LEO will have a behaviour\todo[inline]{Max: Or behavior? It's british or american english right?} close to EFT, while trying to minimize evictions.
