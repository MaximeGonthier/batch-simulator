...
that no two jobs are executed simultaneously on the same core. The
true computation of the job takes a time $\duration(J_i)$, however if a
job does not terminates before its deadline $t_k+\walltime(J_i)$, it
...
when the load of the platform (measured as the number of \todo{nodes/cores?}jobs running
