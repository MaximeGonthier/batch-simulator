...
%~ \usepackage{amsmath}
%~ \usepackage{amsthm}
%~ \usepackage{graphicx,color,amssymb, url, xcolor, tikz, pgf, float, subcaption, algorithm, tabularx, multirow}
...
Maxime Gonthier
  ...
  \thanks[sfn]{LIP, CNRS, ENS de Lyon, Inria \& Université Claude-Bernard Lyon 1}%
...
\and Samuel Thibault\thanks{LaBRI, Université de Bordeaux, CNRS, Inria Bordeaux – Sud-Ouest} 
\and Elisabeth Larsson\thanks[sfn2]{Department of Information Technology, Division of Scientific Computing, Uppsala University} 
\and Carl Nettelblad\thanksref{sfn2}
}

%% Ceci apparait sur chaque page paire.
\authorhead{Gonthier \& Marchal \& Thibault \& Larsson \& Nettelblad}
%% titre francais long
\RRtitle{Ordonnancement de charges de travail intensives en entrées-sorties par lot utilisant la localité des données}
%% English title
\RRetitle{Locality-aware batch scheduling of I/O intensive workloads}
%%
\titlehead{Locality-aware batch scheduling of I/O intensive workloads}
%%
...
\RRresume{
  ...
  %~ Cela peut survenir dans les situations intense en données d'entrée.
  Dans cetaines situations, la quantité de tel travaux peut être importante.
  Le même fichier d'entrée peut être chargé plusieurs fois,
  menant à une perte de ressources.
  
  Nous étudions la création d'un \textit{ordonnanceur de travaux utilisant la localité
  ...
  et également capable de ré-utiliser des fichiers déjà chargés afin de \textit{limiter
  les transferts de données et ainsi réduire le temps d'attente
  des travaux}.
   
  Nous présentons trois ordonnanceurs capable de distribuer la 
  charge entre les nœuds de calculs ainsi que de ré-utiliser 
  le plus possible
  ...
  la localité des données pour ordonner les travaux permet une réduction
...
%%
\RRmotcle{Travaux partageant des entrées,
Localité des données,
Ordonnancement de travaux,
Analyse de données haute performance}
\RRkeyword{Job input sharing,
Data-aware,
Job scheduling,
High Performance Data Analytics}
%%
%~ \RRprojet{ROMA - STORM}  % cas d'un seul projet
\RRprojets{ROMA and STORM}
%%
...
\todo[inline]{Elisabeth: I think that in principle this paragraph is enough together with the picture. We need to clarify that we consider jobs submitted on a specific day rather than jobs running a specific day. Suggestion: In order to avoid simulating the whole workload (one year) but to
target different scenarios, we randomly select a number of days (44 days) and
extract the jobs submitted on these days from the logs. However,
to simulate these jobs in a realistic, steady-state operation of the
platform, we also consider jobs submitted before and after the day
under consideration. More precisely, we proceed as illustrated in Figure~\ref{fig.workload}).}
...
\todo[inline]{LM: subsection'titles are a bit to literary to me ;)}
...
\begin{figure}\centering\includegraphics[width=0.9\linewidth]{../MBSS/plot/Results_FCFS_Score_Backfill_2022-07-16->2022-07-16_V10000_Mean_Stretch_Total_waiting_for_a_load_time_and_transfer_time_450_128_32_256_4_1024.pdf}\caption{Average stretch of all jobs evaluated and total transfer time on July 16.}\label{stretch.07-16}\end{figure}
%~ \begin{figure}\centering\includegraphics[width=0.9\linewidth]{../MBSS/plot/Results_FCFS_Score_Backfill_2022-07-16->2022-07-16_V10000_Mean_Stretch_450_128_32_256_4_1024.pdf}\caption{Average stretch of all jobs evaluated on July 16.}\label{stretch.07-16}\end{figure}
%~ \begin{figure}\centering\includegraphics[width=0.9\linewidth]{../MBSS/plot/Results_FCFS_Score_Backfill_2022-07-16->2022-07-16_V10000_Total_waiting_for_a_load_time_and_transfer_time_450_128_32_256_4_1024.pdf}\caption{Amount of time spent waiting for a file to be available on July 16.}\label{load.07-16}\end{figure}
%~ \begin{figure}\centering\includegraphics[width=0.6\linewidth]{../MBSS/plot/Results_FCFS_Score_Backfill_2022-07-16->2022-07-16_V10000_Number_of_data_reuse_450_128_32_256_4_1024.pdf}\caption{Number of evaluated jobs re-using a data on July 16.}\label{reuse.07-16}\end{figure}
...
underloaded cluster. 
%On Figure~\ref{stretch.07-16} and other similar plots, the lower the value is, the better.\todo[inline]{Max: Do we need to explain that the lower the better on these figures? LM: I would not add this here. Max: Should we add it elsewhere or I just delete it?}
...
%\todo[inline]{Max: I re-wrote this part after deleting the plot showing the number of jobs re-using a file. Is it ok?LM: ok}
...
\begin{figure}\centering\includegraphics[width=0.9\linewidth]{../MBSS/plot/Cluster_usage/2022-07-16->2022-07-16_V10000_Fcfs_Used_nodes_Reduced_450_128_32_256_4_1024_core_by_core.pdf}\caption{Visualization of the utilization rate of the cluster on the workload of July 16 with FCFS.}\label{07_16_cluster_usage_fcfs}\end{figure}
\begin{figure}\centering\includegraphics[width=0.9\linewidth]{../MBSS/plot/Cluster_usage/2022-07-16->2022-07-16_V10000_Fcfs_with_a_score_x500_x1_x0_x0_Used_nodes_Reduced_450_128_32_256_4_1024_core_by_core.pdf}\caption{Visualization of the utilization rate of the cluster on the workload of July 16 with LEA.}\label{07_16_cluster_usage_lea}\end{figure}
\begin{figure}\begin{subfigure}[b]{0.49\linewidth}\centering\includegraphics[width=0.9\linewidth]{../MBSS/plot/Stretch_times/Stretch_times_FCFS_LEA_2022-07-16->2022-07-16_V10000_450_128_32_256_4_1024.pdf}\caption{With LEA.}\label{07_16_fcfs_vs_lea}\end{subfigure}
...
%\todo[inline]{Max: We use $t_k$ as the "earliest available time to compute x cores on node k" in the schedulers section. Can we use it here? LM: Yes}
...
\begin{figure}\centering\includegraphics[width=0.9\linewidth]{../MBSS/plot/Results_FCFS_Score_Backfill_2022-09-09->2022-09-09_V10000_Mean_Stretch_Total_waiting_for_a_load_time_and_transfer_time_450_128_32_256_4_1024.pdf}\caption{Average stretch of all jobs evaluated and total load time on September 09.}\label{stretch.09-09}\end{figure}
%~ \begin{figure}\centering\includegraphics[width=0.9\linewidth]{../MBSS/plot/Results_FCFS_Score_Backfill_2022-09-09->2022-09-09_V10000_Mean_Stretch_450_128_32_256_4_1024.pdf}\caption{Average stretch of all jobs evaluated on September 09.}
%~ \label{stretch.09-09}\end{figure}
%~ \begin{figure}\centering\includegraphics[width=0.9\linewidth]{../MBSS/plot/Results_FCFS_Score_Backfill_2022-09-09->2022-09-09_V10000_Total_waiting_for_a_load_time_and_transfer_time_450_128_32_256_4_1024.pdf}\caption{Amount of time spent waiting for a file to be available on September 09.}
...
\begin{figure}\centering\includegraphics[width=0.9\linewidth]{../MBSS/plot/Cluster_usage/2022-09-09->2022-09-09_V10000_Fcfs_Used_nodes_Reduced_450_128_32_256_4_1024_core_by_core.pdf}\caption{Visualization of the utilization rate of the cluster on the workload of September 9 with FCFS.}\label{cluster_09-09_fcfs}\end{figure}
\begin{figure}\centering\includegraphics[width=0.9\linewidth]{../MBSS/plot/Cluster_usage/2022-09-09->2022-09-09_V10000_Fcfs_with_a_score_mixed_strategy_x500_x1_x0_x0_Used_nodes_Reduced_450_128_32_256_4_1024_core_by_core.pdf}\caption{Visualization of the utilization rate of the cluster on the workload of September 9 with LEM.}\label{cluster_09-09_lem}
\end{figure}
...
%\todo[inline]{Sam: "then why did you choose 80\%? What would choosing 100\% have gotten?". Max: Okay I answered in the next 3 sentences. Is it alright?}
...
%(see the different cluster's usage in Sections~\ref{sec.07-16}, \ref{sec.09-09}, \ref{sec.03-26} and \ref{sec.08-16}).
...
\begin{figure}\centering\includegraphics[width=0.9\linewidth]{../MBSS/plot/Results_FCFS_Score_Backfill_2022-03-26->2022-03-26_V10000_Mean_Stretch_Total_waiting_for_a_load_time_and_transfer_time_450_128_32_256_4_1024.pdf}\caption{Average stretch of all jobs evaluated and total load time on March 26.}\label{stretch.03-26}\end{figure}
%~ \begin{figure}\centering\includegraphics[width=0.9\linewidth]{../MBSS/plot/Results_FCFS_Score_Backfill_2022-03-26->2022-03-26_V10000_Mean_Stretch_450_128_32_256_4_1024.pdf}\caption{Average stretch of all jobs evaluated on March 26.}\label{stretch.03-26}\end{figure}
%~ \begin{figure}\centering\includegraphics[width=0.9\linewidth]{../MBSS/plot/Results_FCFS_Score_Backfill_2022-03-26->2022-03-26_V10000_Number_of_data_reuse_450_128_32_256_4_1024.pdf}\caption{Number of evaluated jobs re-using a data on March 26.}\label{reuse.03-26}\end{figure}
\begin{figure}\centering\includegraphics[width=0.9\linewidth]{../MBSS/plot/Cluster_usage/2022-03-26->2022-03-26_V10000_Fcfs_Used_nodes_Reduced_450_128_32_256_4_1024_core_by_core.pdf}\caption{Visualization of the utilization rate of the cluster on the workload of March 26 with FCFS.}\label{cluster_usage.03-26_fcfs}\end{figure}
\begin{figure}\centering\includegraphics[width=0.9\linewidth]{../MBSS/plot/Stretch_times/Stretch_times_FCFS_LEM_2022-03-26->2022-03-26_V10000_450_128_32_256_4_1024.pdf}\caption{Stretch times of each job of LEM compared to FCFS on the workload of March 26.}\label{vs_fcfs_lem_03-26}\end{figure}
...
\begin{figure}\centering\includegraphics[width=0.9\linewidth]{../MBSS/plot/Results_FCFS_Score_Backfill_2022-08-16->2022-08-16_V10000_Mean_Stretch_Total_waiting_for_a_load_time_and_transfer_time_450_128_32_256_4_1024.pdf}\caption{Average stretch of all jobs evaluated and total load time on August 16.}\label{stretch.08-16}\end{figure}
%~ \begin{figure}\centering\includegraphics[width=0.9\linewidth]{../MBSS/plot/Results_FCFS_Score_Backfill_2022-08-16->2022-08-16_V10000_Mean_Stretch_450_128_32_256_4_1024.pdf}\caption{Average stretch of all jobs evaluated on August 16.}\label{stretch.08-16}\end{figure}
%~ \begin{figure}\centering\includegraphics[width=0.9\linewidth]{../MBSS/plot/Cluster_usage/2022-08-16->2022-08-16_V10000_Fcfs_Used_nodes_Reduced_450_128_32_256_4_1024_core_by_core.pdf}\caption{Visualization of the utilization rate of the cluster on the workload of August 16 with FCFS.}\label{cluster_usage.08-16_fcfs}\end{figure}
%~ \begin{figure}\centering\includegraphics[width=0.9\linewidth]{../MBSS/plot/Cluster_usage/2022-08-16->2022-08-16_V10000_Fcfs_with_a_score_mixed_strategy_x500_x1_x0_x0_Used_nodes_Reduced_450_128_32_256_4_1024_core_by_core.pdf}\caption{Visualization of the utilization rate of the cluster on the workload of August 16 with LEM.\todo[inline]{Max: I think this figure can be removed, I don't really use it in the explanations.}}\label{cluster_usage.08-16_lem}\end{figure}
...
\begin{figure}\centering\includegraphics[width=0.9\linewidth]{../MBSS/plot/Boxplot/box_plot_mean_stretch_all_workloads.pdf}\caption{Mean stretch's improvement from FCFS on all evaluated workloads. The whiskers are the octiles. The solid green line represent the median and the dashed one the mean. 6 outliers for LEA and LEM (with maximum value at 33.5) are not depicted for better readability.}\label{boxplot.all}\end{figure}
\begin{figure}\centering\includegraphics[width=0.9\linewidth]{../MBSS/plot/ECDF/ecdf_mean_stretch_all_workloads.pdf}\caption{Empirical distribution function of the mean stretch's improvement from FCFS on all evaluated workloads.}\label{ecdf}\end{figure}
\begin{figure}\centering\includegraphics[width=0.9\linewidth]{../MBSS/plot/BF_AND_NON_BF_Results_FCFS_Score_Backfill_2022-01-28->2022-01-28_V10000_Mean_Stretch_450_128_32_256_4_1024.pdf}\caption{Average stretch of all jobs evaluated on January 28 with (circles) and without (crosses) backfilling.\todo[inline]{Max: Do you want to keep this or I put the old one with bars and hashed bars for backfilling? LM: put the same cross for non-BF strategy, and a different symbol (circle) for those with BF}}\label{stretch.01-28}\end{figure}
\begin{figure}\centering\includegraphics[width=0.9\linewidth]{../MBSS/plot/Boxplot/box_plot_mean_stretch_all_workloads_bf.pdf}\caption{Mean stretch's improvement from FCFS-BF on all evaluated workloads.}\label{boxplot.all_bf}\end{figure}
...
\todo[inline]{LM: I think if we need space, we could remove the empirical distribution (Figure~\ref{ecdf}) as well as the following paragraph.}
...
%\todo[inline]{Max: Proposition for a conclusion. It's a draft, please do not hesitate to completely modify it.}
