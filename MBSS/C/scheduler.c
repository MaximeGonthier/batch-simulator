			...
			for (i = first_node_size_to_choose_from; i <= last_node_size_to_choose_from; i++)
			{
				...
				{						
					...
					if (min_score == -1 or earliest_available_time < min_score)
						...
						{
							time_to_load_file, is_being_loaded = is_my_file_on_node_at_certain_time_and_transfer_time(earliest_available_time, n, t, j->data, j->data_size); /* Use the intervals in each data to get this info. */
						}
