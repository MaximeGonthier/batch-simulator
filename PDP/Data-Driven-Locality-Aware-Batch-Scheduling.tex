...
%~ email address}
%~ \and
%~ \IEEEauthorblockN{5\textsuperscript{th} Author}
%~ \IEEEauthorblockA{\textit{dept. name of organization} \\
%~ City, Country \\
%~ email address}
%~ }
...
input database
%\footnote{See \url{https://benlangmead.github.io/aws-indexes/k2}}
are often used,
...
LEA, LEO and LEM greatly reduces the transfer time thanks to locality.\mx{A re dire mieux}
This result in Figure~\ref{smallhist.43} in better improvement.
...
We report here the results of the 5 schedulers on the 12 evaluated weeks.
Figures~9 to 12 represent the aggregated
...
% Explication boxplots
Figure~\ref{boxplot.all} depicts 
the distribution of the improvement of each \us stretch, compared to FCFS.
In other words we represent $\mathit{Stretch\_FCFS}/\mathit{Stretch\_Scheduler}$ for each \us and each scheduler.
