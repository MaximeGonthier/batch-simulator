...
void print_data_intervals(struct Node_List** list, int t)
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		while (n != NULL)
		{
			...
			#else
			struct Data* d = n->data->head;
			printf("Temp Data:"); fflush(stdout);
			while (d != NULL)
			{
				printf(" %d (", d->unique_id); fflush(stdout);
				struct Interval* i = d->intervals->head;
...
void print_csv(struct To_Print* head_to_print)
{
	...
	//~ file_to_open_2 = malloc(size_file_to_open_2*sizeof(char));
	...
	//~ strcat(file_to_open_2, scheduler);
	//~ strcat(file_to_open_2, ".txt");
	//~ FILE* f_bounded_stretch2 = fopen(file_to_open_2, "w");
	//~ if (!f_bounded_stretch2)
	//~ {
		//~ perror("Error opening file.\n");
		//~ exit(EXIT_FAILURE);
	//~ }
	//~ fprintf(f_bounded_stretch2, "%f", mean_flow_stretch_with_a_minimum);
	//~ fclose(f_bounded_stretch2);
...
void print_holes(struct Node_List** list)
{
	...
	for (i = 0; i < 3; i++)
	{
		...
		while (n != NULL)
		{
			...
			while (c != NULL)
			{
				printf("Hole on node %d core %d. Next job start at time %d.\n", n->unique_id, c->unique_id, c->start_time_of_the_hole); fflush(stdout);
